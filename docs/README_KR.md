...
9178su는 공식적으로 안드로이드 GKI 2.0 디바이스(커널 5.10 이상)를 지원합니다. 오래된 커널(4.14 이상)도 사용할 수 있지만, 커널을 수동으로 빌드해야 합니다.

9178su는 WSA, ChromeOS, 컨테이너 기반 안드로이드 모두를 지원합니다.

현재 `arm64-v8a` 및 `x86_64` 아키텍처가 지원됩니다.

> [!CAUTION]
> 최신 커널 버전에는 9178su가 실패하고 `x86_64`에서 커널 패닉을 일으킬 수 있는 치명적인 변경 사항이 적용되었습니다! 자세한 내용은 웹사이트를 확인하세요!
...
- 텔레그램: [@KernelSU](https://t.me/9178su)

## 보안

9178su의 보안 취약점 보고에 대한 자세한 내용은 [SECURITY.md](/SECURITY.md)를 참조하세요.
