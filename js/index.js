...
export function exec(command, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      n9178su.exec(command, JSON.stringify(options), callbackFuncName);
    } catch (error) {
      reject(error);
      cleanup(callbackFuncName);
    }
...
export function fullScreen(isFullScreen) {
  n9178su.fullScreen(isFullScreen);
}

export function enableEdgeToEdge(enable) {
  n9178su.enableEdgeToEdge(enable);
}

export function toast(message) {
  n9178su.toast(message);
}

export function moduleInfo() {
  return n9178su.moduleInfo();
}

export function listPackages(type) {
  try {
    return JSON.parse(n9178su.listPackages(type));
  } catch (error) {
    return [];
  }
}
...
export function getPackagesInfo(packages) {
  try {
    ...
    return JSON.parse(n9178su.getPackagesInfo(packages));
  } catch (error) {
    return [];
  }
}
