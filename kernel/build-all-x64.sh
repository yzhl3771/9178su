...
for i in "${!KMIS[@]}"; do
    ...
    if [ "$kmi" == "android17-6.18" ]; then
        make -C "$KDIR" "M=$MDIR" "MO=$ODIR" compile_commands.json modules CONFIG_N9178SU=m CONFIG_N9178SU_X86_PATCH_SYSCALL_DISPATCHER=y
    else
        make -C "$KDIR" "M=$ODIR" "src=$MDIR" compile_commands.json modules CONFIG_N9178SU=m CONFIG_N9178SU_X86_PATCH_SYSCALL_DISPATCHER=y
    fi
