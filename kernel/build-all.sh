...
for kmi in $KMIS; do
    ...
    if ddk build "$kmi" "ODIR=$ODIR" -e CONFIG_N9178SU=m; then
        if [ -f "$ODIR/9178su.ko" ]; then
            cp "$ODIR/9178su.ko" "9178su-${kmi}.ko"
            llvm-strip -d "9178su-${kmi}.ko"
            echo "✓ Built 9178su-${kmi}.ko"
        fi
    else
