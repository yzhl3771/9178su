...
{
    static const char kLibAdbRoot[] = "/data/adb/9178su/lib/libadbroot.so";
    struct path path;
    long ret = kern_path(kLibAdbRoot, 0, &path);
    if (ret < 0) {
        if (ret == -ENOENT) {
            pr_err("libadbroot.so not exists, skip adb root. Please run `n9178sud install`\n");
            ret = 0;
        } else {
...
{
    static const char kLdPreload[] = "LD_PRELOAD=/data/adb/9178su/lib/libadbroot.so";
    static const char kLdLibraryPath[] = "LD_LIBRARY_PATH=/data/adb/9178su/lib";
