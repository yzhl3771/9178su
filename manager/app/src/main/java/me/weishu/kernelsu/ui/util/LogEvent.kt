...
fun getBugreportFile(context: Context): File {
    ...
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C /data/adb/9178su/log .").exec()
    ...
    shell.newJob().add("du -sh /data/adb/9178su/* > ${ksuFileSize.absolutePath}").exec()
    shell.newJob().add("cp /data/system/packages.list ${appListFile.absolutePath}").exec()
    shell.newJob().add("getprop > ${propFile.absolutePath}").exec()
    shell.newJob().add("cp /data/adb/9178su/.allowlist ${allowListFile.absolutePath}").exec()
    ...
    val targetFile = File(context.cacheDir, "9178su_bugreport_${current}.tar.gz")
