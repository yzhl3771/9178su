...
mod android {
    ...
    pub fn ensure_binaries(ignore_if_exist: bool) -> anyhow::Result<()> {
        for file in Asset::iter() {
            if file == "n9178suinit" || file.ends_with(".ko") {
        ...
        std::os::unix::fs::symlink("/data/adb/n9178sud", resetprop_link)?;
...
pub fn list_supported_kmi() -> std::vec::Vec<std::string::String> {
    ...
    for file in Asset::iter() {
        // kmi_name = "xxx_9178su.ko"
        if let Some(kmi) = file.strip_suffix("_9178su.ko") {
