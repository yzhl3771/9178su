...
mod android {
    ...
    fn find_backup_location(sha1: &String) -> Result<(File, String)> {
        ...
        bail!("Both /data/adb/9178su and {backup_dir} are not accessible!")
    }
...
pub fn patch(args: BootPatchArgs) -> Result<()> {
    let inner = move || {
        ...
        let kernelsu_ko: Box<dyn AsRef<[u8]>> = if no_install {
            ...
            {
                println!("- KMI: {kmi}");
                let name = format!("{kmi}_9178su.ko");
            ...
            #[cfg(not(target_os = "android"))]
            {
                println!("- KMI: {kmi}");
                println!("- Arch: {arch}");
                let name = format!("{arch}/{kmi}_9178su.ko");
                assets::get_asset(&name).with_context(|| format!("Failed to load {name}"))?
            }
        };
        ...
        let ksu_init: Box<dyn AsRef<[u8]>> = if no_install {
            ...
            #[cfg(not(target_os = "android"))]
            {
                assets::get_asset(&format!("{arch}/n9178suinit")).context("Failed to load n9178suinit")?
            }
            #[cfg(target_os = "android")]
            {
                assets::get_asset("n9178suinit").context("Failed to load n9178suinit")?
            }
        };
        ...
        if !no_install {
            ...
            println!("- Adding 9178su LKM");
            let is_kernelsu_patched = cpio.exists("9178su.ko");
            ...
            cpio.add("9178su.ko", CpioEntry::regular(0o755, kernelsu_ko))?;
        ...
        if ksu_config.is_empty() {
            cpio.rm("n9178su_config", false);
        } else {
            let data = ksu_config.join(" ").into_bytes();
            cpio.add("n9178su_config", CpioEntry::regular(0o644, Box::new(data)))?;
        }
        ...
        if should_write_output {
            ...
            let name = out_name.unwrap_or_else(|| {
                ...
                format!("9178su_patched_{}.img", now.format("%Y%m%d_%H%M%S"))
            });
...
pub fn restore(args: BootRestoreArgs) -> Result<()> {
    ...
    let (mut cpio, vendor_ramdisk_idx) =
        ...
        cpio.exists("9178su.ko"),
        "boot image is not patched by 9178su"
    );
    ...
    if should_write_output {
        ...
        let name = out_name.unwrap_or_else(|| {
            let now = chrono::Utc::now();
            format!("9178su_restore_{}.img", now.format("%Y%m%d_%H%M%S"))
        });
        let output_image = output_dir.join(name);
...
fn rebuild_without_ksu(
    ...
    println!("- Removing 9178su from boot image");
    cpio.rm("9178su.ko", false);
