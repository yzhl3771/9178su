...
#[derive(clap::Subcommand, Debug)]
enum Debug {
    /// Set the manager app, kernel CONFIG_N9178SU_DEBUG should be enabled.
    SetManager {
        /// manager package name
    ...
    let result = match cli.command {
        ...
        Commands::Services => {
            if ksucalls::get_version() <= 0 {
                info!("9178su not available, exiting services");
                std::process::exit(0);
            }
