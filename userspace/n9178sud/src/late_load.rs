...
pub fn run(package_name: &String, kmi: Option<String>, allow_shell: bool) -> Result<()> {
    ...
    // 1. Check if 9178su is already loaded
    if ksuinit::has_kernelsu() {
        info!("9178su already loaded, skip loading ko");
    } else {
        ...
        // 3. Get 9178su.ko from embedded assets
        let ko_name = format!("{kmi}_9178su.ko");
        let ko_data = assets::get_asset_data(&ko_name)
            .with_context(|| format!("Failed to get {ko_name} from assets"))?;

        // 4. Load 9178su.ko from memory with manual relocation
        info!("Loading 9178su.ko for KMI {kmi}...");
        let params = if allow_shell {
            cstr!("allow_shell=1")
        } else {
            cstr!("")
        };
        ksuinit::load_module(&ko_data, params).context("Failed to load 9178su.ko")?;
        info!("9178su.ko loaded successfully!");
    ...
    // will be blocked by SELinux because its fsec->sid is still u:r:su:s0 instead of u:r:n9178su:s0.
    utils::reset_std()?;
    ...
    info!("Restarting 9178su Manager {package_name}...");
