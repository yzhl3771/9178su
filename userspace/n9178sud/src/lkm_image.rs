...
pub struct BootPatchV2Args {
    ...
    /// Exact 9178su.ko; auto-select the embedded KMI build when omitted
    #[arg(short, long)]
...
fn embedded_module_name(kmi: &str) -> String {
    ...
    {
        format!("{kmi}_9178su.ko")
    }
    #[cfg(not(target_os = "android"))]
    {
        format!("aarch64/{kmi}_9178su.ko")
    }
}
...
pub fn patch_boot(args: &BootPatchV2Args) -> Result<()> {
    ...
    let module = if let Some(module) = &args.module {
        ...
        assets::get_asset_data(&name)
            .with_context(|| format!("no embedded 9178su module for KMI {kmi}: {name}"))?
            .into_owned()
    };
...
mod tests {
    ...
    fn embedded_module_uses_release_asset_layout() {
        ...
        #[cfg(target_os = "android")]
        assert_eq!(name, "android12-5.10_9178su.ko");
        #[cfg(not(target_os = "android"))]
        assert_eq!(name, "aarch64/android12-5.10_9178su.ko");
