...
pub fn root_shell() -> Result<()> {
    ...
    opts.optflag(
        "",
        "ksu-no-new-privs",
        "Prevent this process (and its children) from privilege re-escalation via 9178su",
    );
    ...
    if matches.opt_present("v") {
        println!("{}:9178su", defs::VERSION_NAME);
        return Ok(());
    }
