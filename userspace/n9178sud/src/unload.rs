...
/// Find PIDs of processes running in the 9178su su domain (u:r:n9178su:s0).
...
pub fn unload() -> Result<()> {
    info!("unload: starting 9178su unload sequence");
    ...
    info!("unload: closing all 9178su fds...");
    close_ksu_fds();

    // 4. delete_module("9178su")
    info!("unload: removing 9178su module...");
    if let Err(e) = rustix::system::delete_module(c"9178su", 0) {
        warn!("unload: delete_module 9178su failed: {e}");
    }
    ...
    // 6. Exit
    info!("unload: done, exiting n9178sud");
    std::process::exit(0);
}
