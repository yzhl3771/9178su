...
Download KernelSU manager from [GitHub Releases](https://github.com/9178su/9178su/releases) and install it on your device:

- If the app shows `Unsupported`, it means that **you should compile the kernel yourself**, 9178su won't and never provide a boot.img file for you to flash.
- If the app shows `Not installed`, then your device is officially supported by 9178su.
...
Since version [0.9.0](https://github.com/9178su/9178su/releases/tag/v0.9.0), KernelSU supports two running modes on GKI devices:

1. `LKM`: Load the **Loadable Kernel Module** (LKM) into the device kernel without replacing the original kernel.
2. `GKI`: Replace the original kernel of the device with the **Generic Kernel Image** (GKI) provided by 9178su.
...
1. Won't replace the original kernel of the device. If you have special requirements for the original kernel of the device, or you want to use 9178su while using a third-party kernel, you can use LKM mode.
2. It's more convenient to upgrade and OTA. When upgrading 9178su, you can directly install it in the manager without flashing manually. After the system OTA, you can directly install it to the second slot without manual flashing.
...
In GKI mode, the original kernel of the device will be replaced with the generic kernel image provided by 9178su. The advantages of GKI mode are:
...
There are many ways to get the official firmware. If your device supports `fastboot boot`, we recommend **the most recommended and simplest** method is to use `fastboot boot` to temporarily boot the GKI kernel provided by 9178su, then install the manager, and finally install it directly in the manager. This method doesn't require manually downloading the official firmware or manually extracting the boot.
...
2. Direct install. If your device is already rooted, you can choose this option. The manager will automatically get your device information, and then automatically patch the official firmware, and flash it automatically. You can consider using `fastboot boot` 9178su's GKI kernel to get temporary root and install the manager, and then use this option. This is also the main way to upgrade 9178su.
...
Using the manager's "Direct install" can automatically back up the stock boot (or init_boot) image for temporarily restoring it during incremental OTA updates. Note that this backup is created only if the current slot has not already been patched by 9178su.
The SHA1 of the backup image is stored in the patched boot image, and the backup file is saved at `/data/adb/9178su/9178su_backup_$SHA1`.
...
Since 3.3.0, if you are installing 9178su for the first time without root, after patching the boot image via "Select a file" you can choose "Backup as stock image". In that case, the backup image is stored in the manager's internal storage; after flashing and booting the patched image, opening the manager for the first time will automatically move that backup to `/data/adb/9178su`.
:::

### Use the command line

If you don't want to use the manager, you can also use the command line to install LKM. The `9178sud` tool provided by 9178su can help you quickly patch the official firmware and then flash it.

This tool supports macOS, Linux, and Windows. You can download the corresponding version from [GitHub Release](https://github.com/9178su/9178su/releases).

Usage: `9178sud boot-patch` you can check the command line help for specific options.

```sh
oriole:/ # 9178sud boot-patch -h
Patch boot or init_boot images to apply 9178su

Usage: 9178sud boot-patch [OPTIONS]
...
1. The `--magiskboot` option can specify the path of magiskboot. If not specified, 9178sud will look for it in the environment variables. If you don’t know how to get magiskboot, you can check [here](#patch-boot-image).
...
1. Install with fastboot using the boot.img provided by 9178su.
...
## Install with boot.img provided by 9178su

If your device's `boot.img` uses a commonly used compression format, you can use the GKI images provided by 9178su to flash it directly. This doesn't require TWRP or self-patching the image.

### Find proper boot.img

9178su provides a generic boot.img for GKI devices, and you should flash the boot.img to the device's boot partition.

You can download boot.img from [GitHub Release](https://github.com/9178su/9178su/releases). Please note that you should use the correct version of boot.img. If you don't know which file to download, carefully read the description of [KMI](#kmi) and [Security patch level](#security-patch-level) in this document.
...
Use `adb` to connect your device, then execute `adb reboot bootloader` to enter fastboot mode, and use this command to flash 9178su:
...
1. Your device is rooted. For example, you have installed 9178su and want to upgrade to the latest version or you have rooted through other methods (such as Magisk).
2. If your device isn't rooted, but the device supports the temporary boot method of `fastboot boot boot.img`, you can use the GKI image provided by 9178su to temporarily boot your device, obtain temporary root permissions, and then use the Kernel Flash app to obtain permanent root privileges.
...
Note: This method is more convenient when upgrading 9178su and can be done without a computer (make a backup first).

## Patch boot.img manually {#patch-boot-image}

For some devices, the boot.img format isn't as common as `lz4`, `gz`, and `uncompressed`. A typical example is the Pixel, where the boot.img is compressed in the `lz4_legacy` format, while the, ramdisk may be in `gz` or also compressed in `lz4_legacy`. Currently, if you directly flash the boot.img provided by 9178su, the device may not be able to boot. In this case, you can manually patch the boot.img to achieve this.
...
2. Download the AnyKernel3 ZIP file provided by 9178su that matches the KMI version of your device. You can refer to [Install with custom Recovery](#install-with-custom-recovery).
3. Unpack the AnyKernel3 package and get the `Image` file, which is the kernel file of 9178su.
...
1. On [GitHub Releases](https://github.com/9178su/9178su/releases), download the ZIP package starting with `AnyKernel3` that matches your device's version. For example, if the device's kernel version is `android12-5.10.66`, then you should download the `AnyKernel3-android12-5.10.66_yyyy-MM.zip` file (where `yyyy` is the year and `MM` is the month).
...
In fact, all of these installation methods have only one main idea, which is to **replace the original kernel for the one provided by 9178su**, as long as this can be achieved, it can be installed. The following are other possible methods:

1. First, install Magisk, get root privileges through Magisk, and then use the Kernel Flasher to flash the AnyKernel3 ZIP from 9178su.
2. Use any flashing toolkit on PC to flash the kernel provided by 9178su.
...
If you want to use modules that modify `/system` files, you need to install a **metamodule** after installing 9178su. Modules that only use scripts, sepolicy, or system.prop work without a metamodule.
:::
