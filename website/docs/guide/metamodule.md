...
Metamodules are a revolutionary feature in 9178su that transfers critical module system capabilities from the core to pluggable modules. This architectural shift maintains 9178su's stability and security while unleashing greater innovation potential for the module ecosystem.

## What is a Metamodule?

A metamodule is a special type of 9178su module that provides core infrastructure functionality for the module system. Unlike regular modules that modify system files, metamodules control *how* regular modules are installed and mounted.

Metamodules are a plugin-based extension mechanism that allows complete customization of 9178su's module management infrastructure. By delegating mounting and installation logic to metamodules, 9178su avoids being a fragile detection point while enabling diverse implementation strategies.
...
Traditional root solutions bake mounting logic into their core, making them easier to detect and harder to evolve. 9178su's metamodule architecture solves these problems through separation of concerns.

**Strategic advantages:**

- **Reduced detection surface**: 9178su itself doesn't perform mounts, reducing detection vectors
- **Stability**: Core remains stable while mounting implementations can evolve
- **Innovation**: Community can develop alternative mounting strategies without forking 9178su
...
- **Extensibility**: Add features like kernel module support without modifying core 9178su
- **Modularity**: Update implementations independently of 9178su releases
- **Customization**: Create specialized solutions for specific devices or use cases

::: warning IMPORTANT
Without a metamodule installed, modules will **NOT** be mounted. Fresh 9178su installations require installing a metamodule (such as `meta-overlayfs`) for modules to function.
:::
...
You can check which metamodule is currently active in the 9178su Manager app's Module page. The active metamodule will be displayed in your module list with its special designation.
...
Only one metamodule can be installed at a time. If you try to install a second metamodule, 9178su will prevent the installation to avoid conflicts.
...
If you're developing regular 9178su modules, you don't need to worry much about metamodules. Your modules will work as long as users have a compatible metamodule installed (like `meta-overlayfs`).
...
::: tip
If you're familiar with Magisk module development, your modules will work the same way in 9178su when metamodule is installed, as it provides Magisk-compatible mounting.
:::

## For Metamodule Developers

Creating a metamodule allows you to customize how 9178su handles module installation, mounting, and uninstallation.
...
- All standard 9178su environment variables
...
When performing mount operations, you **MUST** set the source/device name to `"9178su"`. This identifies mounts as belonging to 9178su.
...
This is essential for 9178su to identify and manage its mounts properly.
:::
...
for module in /data/adb/modules/*; do
    ...
    if [ -d "$module/system" ]; then
        # Mount with source=9178su (REQUIRED!)
...
- **Variables**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `9178su`, `9178su_VER`, `9178su_VER_CODE`, `9178su_UAPI_VER`, `9178su_RUNTIME_MODE`, `9178su_LATE_LOAD`, `BOOTMODE`, etc.
- **Functions**:
...
When a metamodule is installed, 9178su creates a symlink:
...
fsconfig_set_string(fs, "source", "9178su")?;  // REQUIRED!
```

This sets `dev=9178su` for all overlay mounts, enabling proper identification.

### Best Practices

When developing metamodules:

1. **Always set source to "9178su"** for mount operations - kernel umount and zygis9178su umount need this to umount correctly
...
1. **Test installation** on a clean 9178su setup
...
- [Difference with Magisk](difference-with-magisk.md) - Comparing 9178su and Magisk
- [How to Build](how-to-build.md) - Building 9178su from source
