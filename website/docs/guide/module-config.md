...
9178su provides a built-in configuration system that allows modules to store persistent or temporary key-value settings. Configurations are stored in a binary format at `/data/adb/9178su/module_configs/<module_id>/` with the following characteristics:
...
All module scripts (`post-fs-data.sh`, `service.sh`, `boot-completed.sh`, etc.) run with the `9178su_MODULE` environment variable set to the module ID. You can use the `9178sud module config` commands to manage your module's configuration:
...
- Configurations are stored in a binary format with magic number `0x4b53554d` ("9178suM") and version validation
...
- Use the `9178sud module config list` command to debug configuration issues
:::
...
9178sud module config set override.description "Custom description shown in the manager"
```
...
Modules can declare which 9178su features they manage using the `manage.<feature>` configuration pattern. The supported features correspond to 9178su's internal `FeatureId` enum:
...
- 9178su manager to detect which modules manage which 9178su features
- Prevention of conflicts when multiple modules try to manage the same feature
- Better coordination between modules and core 9178su functionality

::: warning SUPPORTED FEATURES ONLY
Only use the predefined feature names listed above (`su_compat`, `kernel_umount`). These correspond to actual 9178su internal features. Using other feature names will not cause errors but serves no functional purpose.
:::
