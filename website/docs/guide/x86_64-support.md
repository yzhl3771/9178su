...
9178su fully supports the `x86_64` architecture. However, due to recent upstream kernel security changes, integrating 9178su on modern `x86_64` kernels requires additional handling so our unified syscall dispatcher can function correctly.
...
9178su's `syscall_hook` mechanism relies on modifying entries in the syscall table so intercepted system calls can be routed to our unified dispatcher. Because the new hardening changes the syscall path, the kernel ignores those syscall table modifications. If 9178su attempts to load and hook the syscall table without handling this limitation correctly, it will fail to route the calls and cleanly abort initialization to prevent a kernel panic.

## How to fix it?

There are two supported ways to handle this syscall hook issue on `x86_64`:

1. Enable the kernel build option `9178su_X86_PATCH_SYSCALL_DISPATCHER`.
...
### Option 1: Enable `9178su_X86_PATCH_SYSCALL_DISPATCHER`

9178su 3.3.0 introduced an official mechanism for `x86_64`: the build option `9178su_X86_PATCH_SYSCALL_DISPATCHER`.

When this option is enabled, 9178su dynamically patches the hardened syscall dispatcher at runtime so the syscall hook can work without requiring the previous kernel source patch set. This is the recommended approach if you are building a kernel with 9178su 3.3.0 or newer.

### Option 2: Apply the original kernel source patches

If you do not want to enable `9178su_X86_PATCH_SYSCALL_DISPATCHER`, you can continue to use the original kernel patch approach.

To make 9178su work on these newer kernels, apply a patch that allows you to bypass this specific syscall hardening.
...
This re-opens the indirect branch attack surface for system calls. **Do not use either solution if you are running a production server or a system where strict side-channel security is critical.** These approaches are intended for testing environments where root access via 9178su is prioritized over this specific hardware vulnerability mitigation.
:::
...
- If you are using 9178su 3.3.0 or newer and can change the 9178su build configuration, enable `9178su_X86_PATCH_SYSCALL_DISPATCHER`.
