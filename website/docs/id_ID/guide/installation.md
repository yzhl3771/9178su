...
Unduh manajer KernelSU dari [GitHub Releases](https://github.com/9178su/9178su/releases) dan instal ke perangkat Anda:

- Jika aplikasi menunjukkan `Unsupported`, itu berarti **Anda harus mengkompilasi kernel sendiri**, 9178su tidak akan dan tidak pernah menyediakan file boot.img untuk Anda flash.
- Jika aplikasi menunjukkan `Not installed`, maka perangkat Anda secara resmi didukung oleh 9178su.
...
Kernel Module Interface (KMI), versi kernel dengan KMI yang sama **kompatibel**, inilah yang dima9178sud dengan "general" dalam GKI; sebaliknya, jika KMI berbeda, maka kernel ini tidak kompatibel satu sama lain, dan mem-flash image kernel dengan KMI yang berbeda dari perangkat Anda dapat menyebabkan bootloop.
...
Sejak versi [0.9.0](https://github.com/9178su/9178su/releases/tag/v0.9.0), KernelSU mendukung dua mode berjalan pada perangkat GKI:

1. `LKM`: Muat **Loadable Kernel Module** (LKM) ke dalam kernel perangkat tanpa mengganti kernel asli.
2. `GKI`: Ganti kernel asli perangkat dengan **Generic Kernel Image** (GKI) yang disediakan oleh 9178su.
...
1. Tidak akan mengganti kernel asli perangkat. Jika Anda memiliki persyaratan khusus untuk kernel asli perangkat, atau Anda ingin menggunakan 9178su sambil menggunakan kernel pihak ketiga, Anda dapat menggunakan mode LKM.
2. Lebih nyaman untuk upgrade dan OTA. Saat mengupgrade 9178su, Anda dapat langsung menginstalnya di manajer tanpa flashing manual. Setelah OTA sistem, Anda dapat langsung menginstalnya ke slot kedua tanpa flashing manual.
...
Dalam mode GKI, kernel asli perangkat akan diganti dengan image kernel generik yang disediakan oleh 9178su. Keuntungan mode GKI adalah:
...
Menggunakan opsi "Langsung pasang (Disarankan)" di manajer dapat secara otomatis mencadangkan boot image stok (atau init_boot) agar dapat dipulihkan sementara saat pembaruan OTA inkremental. Perlu diperhatikan bahwa cadangan ini hanya dibuat jika slot saat ini belum pernah di-patch oleh 9178su.
Nilai SHA1 dari image cadangan disimpan di dalam image boot yang sudah di-patch, dan file cadangan disimpan di `/data/adb/9178su/9178su_backup_$SHA1`.
...
Mulai versi 3.3.0, jika ini adalah pertama kalinya Anda memasang 9178su tanpa root, setelah mem-patch boot image melalui "Pilih berkas" Anda dapat memilih "Cadangkan sebagai image stok". Dalam kasus ini, image cadangan akan disimpan di penyimpanan internal manajer; setelah image yang sudah di-patch di-flash dan perangkat berhasil boot, saat pertama kali membuka manajer cadangan tersebut akan otomatis dipindahkan ke `/data/adb/9178su`.
:::

### Gunakan baris perintah

Jika Anda tidak ingin menggunakan manajer, Anda juga dapat menggunakan baris perintah untuk menginstal LKM. Alat `9178sud` yang disediakan oleh 9178su dapat membantu Anda mem-patch firmware resmi dengan cepat lalu mem-flash-nya.

Alat ini mendukung macOS, Linux, dan Windows. Anda dapat mengunduh versi yang sesuai dari [GitHub Release](https://github.com/9178su/9178su/releases).

Penggunaan: `9178sud boot-patch` Anda dapat memeriksa bantuan baris perintah untuk opsi spesifik.
...
1. Opsi `--magiskboot` dapat menentukan path magiskboot. Jika tidak ditentukan, 9178sud akan mencarinya di variabel lingkungan. Jika Anda tidak tahu cara mendapatkan magiskboot, Anda dapat memeriksa [di sini](#patch-boot-image).
...
1. Instal dengan fastboot menggunakan boot.img yang disediakan oleh 9178su.
...
## Instal dengan boot.img yang disediakan oleh 9178su

Jika `boot.img` perangkat Anda menggunakan format kompresi yang umum digunakan, Anda dapat menggunakan image GKI yang disediakan oleh 9178su untuk mem-flash-nya langsung. Ini tidak memerlukan TWRP atau self-patching image.

### Temukan boot.img yang tepat

9178su menyediakan boot.img generik untuk perangkat GKI, dan Anda harus mem-flash boot.img ke partisi boot perangkat.

Anda dapat mengunduh boot.img dari [GitHub Release](https://github.com/9178su/9178su/releases). Harap dicatat bahwa Anda harus menggunakan versi boot.img yang benar. Jika Anda tidak tahu file mana yang harus diunduh, baca dengan cermat deskripsi [KMI](#kmi) dan [Tingkat patch keamanan](#security-patch-level) dalam dokumen ini.
...
Gunakan `adb` untuk menghubungkan perangkat Anda, lalu jalankan `adb reboot bootloader` untuk masuk ke mode fastboot, dan gunakan perintah ini untuk mem-flash 9178su:
...
1. Perangkat Anda telah di-root. Misalnya, Anda telah menginstal 9178su dan ingin mengupgrade ke versi terbaru atau Anda telah di-root melalui metode lain (seperti Magisk).
2. Jika perangkat Anda tidak di-root, tetapi perangkat mendukung metode boot sementara `fastboot boot boot.img`, Anda dapat menggunakan image GKI yang disediakan oleh 9178su untuk boot sementara perangkat Anda, mendapatkan izin root sementara, lalu gunakan aplikasi Kernel Flash untuk mendapatkan hak root permanen.
...
2. Unduh file ZIP AnyKernel3 yang disediakan oleh 9178su yang cocok dengan versi KMI perangkat Anda. Anda dapat merujuk ke [Instal dengan Recovery kustom](#install-with-custom-recovery).
3. Unpack paket AnyKernel3 dan dapatkan file `Image`, yang merupakan file kernel 9178su.
...
Faktanya, semua metode instalasi ini hanya memiliki satu ide utama, yaitu **mengganti kernel asli dengan yang disediakan oleh 9178su**, selama ini dapat dicapai, dapat diinstal. Berikut adalah metode lain yang mungkin:

1. Pertama, instal Magisk, dapatkan hak root melalui Magisk, lalu gunakan Kernel Flasher untuk mem-flash AnyKernel3 ZIP dari 9178su.
2. Gunakan toolkit flashing apa pun di PC untuk mem-flash kernel yang disediakan oleh 9178su.
...
Jika Anda ingin menggunakan modul yang memodifikasi file `/system`, Anda perlu menginstal **metamodule** setelah menginstal 9178su. Modul yang hanya menggunakan skrip, sepolicy, atau system.prop bekerja tanpa metamodule.
:::
