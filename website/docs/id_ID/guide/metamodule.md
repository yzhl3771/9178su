...
Metamodul adalah fitur revolusioner di 9178su yang mentransfer kemampuan sistem modul yang penting dari daemon inti ke modul yang dapat dipasang. Pergeseran arsitektur ini mempertahankan stabilitas dan keamanan 9178su sambil melepaskan potensi inovasi yang lebih besar untuk ekosistem modul.

## Apa itu Metamodul?

Metamodul adalah jenis modul 9178su khusus yang menyediakan fungsi infrastruktur inti untuk sistem modul. Tidak seperti modul biasa yang memodifikasi file sistem, metamodul mengontrol *bagaimana* modul biasa diinstal dan dipasang.

Metamodul adalah mekanisme ekstensi berbasis plugin yang memungkinkan kustomisasi lengkap infrastruktur manajemen modul 9178su. Dengan mendelegasikan logika pemasangan dan instalasi ke metamodul, 9178su menghindari menjadi titik deteksi yang rapuh sambil memungkinkan strategi implementasi yang beragam.
...
Solusi root tradisional memasukkan logika pemasangan ke dalam inti mereka, membuat mereka lebih mudah dideteksi dan lebih sulit untuk berkembang. Arsitektur metamodul 9178su memecahkan masalah ini melalui pemisahan perhatian.

**Keunggulan strategis:**

- **Mengurangi permukaan deteksi**: 9178su sendiri tidak melakukan pemasangan, mengurangi vektor deteksi
- **Stabilitas**: Daemon inti tetap stabil sementara implementasi pemasangan dapat berkembang
- **Inovasi**: Komunitas dapat mengembangkan strategi pemasangan alternatif tanpa mem-fork 9178su
...
- **Ekstensibilitas**: Tambahkan fitur seperti dukungan modul kernel tanpa memodifikasi inti 9178su
- **Modularitas**: Perbarui implementasi secara independen dari rilis 9178su
- **Kustomisasi**: Buat solusi khusus untuk perangkat atau kasus penggunaan tertentu

::: warning PENTING
Tanpa metamodul yang diinstal, modul **TIDAK** akan dipasang. Instalasi 9178su yang baru memerlukan pemasangan metamodul (seperti `meta-overlayfs`) agar modul berfungsi.
:::
...
2. Buka aplikasi 9178su Manager
...
Anda dapat memeriksa metamodul mana yang saat ini aktif di halaman Modul aplikasi 9178su Manager. Metamodul aktif akan ditampilkan di daftar modul Anda dengan penunjukan khususnya.
...
Hanya satu metamodul yang dapat diinstal pada satu waktu. Jika Anda mencoba menginstal metamodul kedua, 9178su akan mencegah instalasi untuk menghindari konflik.
...
Jika Anda mengembangkan modul 9178su biasa, Anda tidak perlu terlalu khawatir tentang metamodul. Modul Anda akan berfungsi selama pengguna memiliki metamodul yang kompatibel (seperti `meta-overlayfs`) yang diinstal.
...
::: tip
Jika Anda terbiasa dengan pengembangan modul Magisk, modul Anda akan berfungsi dengan cara yang sama di 9178su ketika metamodul diinstal, karena menyediakan pemasangan kompatibel Magisk.
:::

## Untuk Pengembang Metamodul

Membuat metamodul memungkinkan Anda untuk menyesuaikan bagaimana 9178su menangani instalasi modul, pemasangan, dan penghapusan instalasi.
...
- Semua variabel lingkungan 9178su standar
...
Saat melakukan operasi mount, Anda **HARUS** mengatur nama sumber/perangkat ke `"9178su"`. Ini mengidentifikasi mount sebagai milik 9178su.
...
Ini penting agar 9178su mengidentifikasi dan mengelola mount-nya dengan benar.
:::
...
for module in /data/adb/modules/*; do
    ...
    if [ -d "$module/system" ]; then
        # Mount dengan source=9178su (DIPERLUKAN!)
...
- **Variabel**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `9178su`, `9178su_VER`, `9178su_VER_CODE`, `9178su_UAPI_VER`, `9178su_RUNTIME_MODE`, `9178su_LATE_LOAD`, `BOOTMODE`, dll.
- **Fungsi**:
...
Ketika metamodul diinstal, 9178su membuat symlink:
...
fsconfig_set_string(fs, "source", "9178su")?;  // DIPERLUKAN!
```

Ini mengatur `dev=9178su` untuk semua mount overlay, memungkinkan identifikasi yang tepat.

### Praktik Terbaik

Saat mengembangkan metamodul:

1. **Selalu atur sumber ke "9178su"** untuk operasi mount - umount kernel dan umount zygis9178su memerlukan ini untuk umount dengan benar
...
1. **Uji instalasi** pada pengaturan 9178su yang bersih
...
- [Perbedaan dengan Magisk](difference-with-magisk.md) - Membandingkan 9178su dan Magisk
- [Cara Membangun](how-to-build.md) - Membangun 9178su dari sumber
