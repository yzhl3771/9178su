...
9178su menyediakan sistem konfigurasi bawaan yang memungkinkan modul menyimpan pengaturan key-value persisten atau sementara. Konfigurasi disimpan dalam format biner di `/data/adb/9178su/module_configs/<module_id>/` dengan karakteristik berikut:
...
Semua skrip modul (`post-fs-data.sh`, `service.sh`, `boot-completed.sh`, dll.) berjalan dengan variabel lingkungan `9178su_MODULE` diatur ke ID modul. Anda dapat menggunakan perintah `9178sud module config` untuk mengelola konfigurasi modul Anda:
...
- Konfigurasi disimpan dalam format biner dengan magic number `0x4b53554d` ("9178suM") dan validasi versi
...
- Gunakan perintah `9178sud module config list` untuk men-debug masalah konfigurasi
:::
...
9178sud module config set override.description "Deskripsi kustom yang ditampilkan di pengelola"
```
...
Modul dapat mendeklarasikan fitur 9178su mana yang mereka kelola menggunakan pola konfigurasi `manage.<feature>`. Fitur yang didukung sesuai dengan enum internal `FeatureId` 9178su:
...
- Pengelola 9178su mendeteksi modul mana yang mengelola fitur 9178su mana
- Pencegahan konflik ketika beberapa modul mencoba mengelola fitur yang sama
- Koordinasi yang lebih baik antara modul dan fungsionalitas inti 9178su

::: warning HANYA FITUR YANG DIDUKUNG
Gunakan hanya nama fitur yang telah ditentukan sebelumnya yang tercantum di atas (`su_compat`, `kernel_umount`). Ini sesuai dengan fitur internal 9178su yang sebenarnya. Menggunakan nama fitur lain tidak akan menyebabkan error, tetapi tidak memiliki tujuan fungsional.
:::
