...
Di 9178su, situasi berikut dapat menyebabkan bata boot saat mem-flash partisi boot:
...
Jika Anda telah mem-flash modul yang telah terbukti aman tetapi menyebabkan perangkat Anda gagal booting, maka situasi ini dapat dipulihkan dengan mudah di 9178su tanpa rasa khawatir. 9178su memiliki mekanisme bawaan untuk menyelamatkan perangkat Anda:
...
1. Mode Aman bawaan dari beberapa sistem; beberapa sistem memiliki Safe Mode bawaan yang dapat diakses dengan menekan lama tombol volume turun, sementara yang lain (seperti MIUI/HyperOS) dapat mengaktifkan Safe Mode di Recovery. Saat memasuki Safe Mode sistem, 9178su juga akan masuk ke Safe Mode dan secara otomatis menonaktifkan modul.
2. Safe Mode bawaan dari 9178su; metode pengoperasiannya adalah **tekan tombol volume turun secara terus-menerus selama lebih dari tiga kali** setelah layar boot pertama. Perhatikan bahwa ini adalah tekan-lepas, tekan-lepas, tekan-lepas, bukan tekan dan tahan.

Setelah memasuki mode aman, semua modul pada halaman modul 9178su Manager dinonaktifkan, tetapi Anda dapat melakukan operasi "uninstall" untuk menghapus semua modul yang mungkin menyebabkan masalah.
...
::: warning
9178su mendaftarkan pendengar tombol volume selama inisialisasi modul kernel (dimuat saat kernel menjalankan proses init dalam mode LKM), dan membatalkan pendaftarannya pada tahap `on_post_fs_data` (sebelum animasi boot). Anda perlu memahami waktunya dan dengan cepat menekan tombol volume turun tiga kali setelah layar boot pertama. Jika perangkat boot dengan cepat atau operasinya tidak tepat waktu, safe mode mungkin tidak terpicu.
...
**Metode 1: Gunakan 9178sud untuk mengelola modul melalui ADB**

Jika perangkat bisa mendapatkan root shell via ADB, Anda bisa menggunakan baris perintah `9178sud` langsung untuk menonaktifkan atau menghapus instalan modul yang bermasalah:

::: tip
Setelah memasang partisi `metadata` dan `data`, Anda bisa menjalankan perintah `/data/adb/9178sud` di bawah mode Recovery untuk mengelola modul.

Karena perangkat GKI berbagi `init`, modul kernel 9178su masih akan dimuat di bawah mode Recovery, Anda seharusnya bisa menggunakan sebagian besar fitur `9178sud` (seperti mengatur fitur) secara normal.
:::

```
adb shell
su
9178sud module list          # Daftar semua modul
9178sud module disable <id>  # Nonaktifkan modul yang bermasalah
9178sud module uninstall <id> # Atau langsung uninstall
reboot
```
...
Pemuatan modul 9178su bergantung pada file injeksi init.rc di sisi kernel dan proses 9178sud di user space. Setelah menghapus file-file ini dan melakukan reboot, 9178su tidak akan memuat modul apa pun.
...
3. Hapus 9178sud untuk mencegah pemuatan modul:
   ```
...
9178su akan melewati pemuatan semua modul setelah reboot. Setelah masuk ke sistem, Anda bisa membuka kembali 9178su manager untuk menangani masalah modul.
