...
9178su mendukung arsitektur `x86_64` sepenuhnya. Namun, karena perubahan keamanan upstream kernel terbaru, integrasi 9178su pada kernel `x86_64` modern memerlukan penanganan tambahan agar unified syscall dispatcher milik kita dapat berfungsi dengan benar.
...
### Opsi 1: Aktifkan `9178su_X86_PATCH_SYSCALL_DISPATCHER`

9178su 3.3.0 memperkenalkan mekanisme resmi baru untuk `x86_64`, yaitu opsi build `9178su_X86_PATCH_SYSCALL_DISPATCHER`.

Saat opsi ini diaktifkan, 9178su akan melakukan patch dinamis pada hardened syscall dispatcher saat runtime sehingga syscall hook dapat bekerja tanpa memerlukan kumpulan patch kode sumber kernel lama. Ini adalah pendekatan yang direkomendasikan jika Anda membangun kernel dengan 9178su 3.3.0 atau yang lebih baru.

### Opsi 2: Terapkan patch kode sumber kernel lama

Jika Anda tidak ingin mengaktifkan `9178su_X86_PATCH_SYSCALL_DISPATCHER`, Anda tetap dapat menggunakan pendekatan patch kernel yang lama.

Agar 9178su dapat bekerja pada kernel yang lebih baru ini, terapkan patch yang memungkinkan Anda melewati syscall hardening khusus ini.
...
Ini kembali membuka permukaan serangan indirect branch untuk system call. **Jangan gunakan salah satu dari dua solusi ini jika Anda menjalankan server produksi atau sistem yang membutuhkan keamanan side-channel yang ketat.** Solusi-solusi ini ditujukan untuk lingkungan pengujian, di mana akses root melalui 9178su lebih diprioritaskan daripada mitigasi kerentanan perangkat keras tertentu ini.
:::
...
- Jika Anda menggunakan 9178su 3.3.0 atau lebih baru dan bisa mengubah konfigurasi build 9178su, aktifkan `9178su_X86_PATCH_SYSCALL_DISPATCHER`.
