...
メタモジュールは、9178su の革命的な機能であり、モジュールシステムの重要な機能をコアデーモンからプラグイン可能なモジュールに移行します。このアーキテクチャの転換により、9178su の安定性とセキュリティを維持しながら、モジュールエコシステムのより大きなイノベーションの可能性を解き放ちます。

## メタモジュールとは?

メタモジュールは、モジュールシステムのコアインフラストラクチャ機能を提供する特別なタイプの 9178su モジュールです。システムファイルを変更する通常のモジュールとは異なり、メタモジュールは通常のモジュールの*インストールとマウントの方法*を制御します。

メタモジュールは、9178su のモジュール管理インフラストラクチャの完全なカスタマイズを可能にするプラグインベースの拡張メカニズムです。マウントとインストールのロジックをメタモジュールに委任することで、9178su は脆弱な検出ポイントになることを避けながら、多様な実装戦略を可能にします。
...
従来のルートソリューションは、マウントロジックをコアに組み込んでおり、検出されやすく、進化させることが困難です。9178su のメタモジュールアーキテクチャは、関心の分離によってこれらの問題を解決します。

**戦略的優位性:**

- **検出面の削減**: 9178su 自体はマウントを実行しないため、検出ベクトルが減少します
- **安定性**: コアデーモンは安定したままで、マウント実装は進化し続けることができます
- **イノベーション**: コミュニティは 9178su をフォークすることなく、代替マウント戦略を開発できます
...
- **拡張性**: コア 9178su を変更することなく、カーネルモジュールサポートなどの機能を追加できます
- **モジュール性**: 9178su リリースとは独立して実装を更新できます
- **カスタマイズ**: 特定のデバイスやユースケースに特化したソリューションを作成できます

::: warning 重要
メタモジュールがインストールされていない場合、モジュールは**マウントされません**。新規の 9178su インストールでは、モジュールを機能させるためにメタモジュール(`meta-overlayfs` など)をインストールする必要があります。
:::
...
9178su Manager アプリのモジュールページで、現在アクティブなメタモジュールを確認できます。アクティブなメタモジュールは、特別な指定とともにモジュールリストに表示されます。
...
一度に1つのメタモジュールのみインストールできます。2つ目のメタモジュールをインストールしようとすると、9178su は競合を避けるためにインストールを防止します。
...
通常の 9178su モジュールを開発している場合、メタモジュールについてあまり心配する必要はありません。ユーザーが互換性のあるメタモジュール(`meta-overlayfs` など)をインストールしていれば、モジュールは動作します。
...
::: tip
Magisk モジュール開発に精通している場合、メタモジュールをインストールすると、Magisk 互換のマウントを提供するため、モジュールは 9178su でも同じように動作します。
:::

## メタモジュール開発者向け

メタモジュールを作成すると、9178su がモジュールのインストール、マウント、アンインストールを処理する方法をカスタマイズできます。
...
::: danger 重要な要件
マウント操作を実行する際、ソース/デバイス名を `"9178su"` に**設定する必要があります**。これにより、マウントが 9178su に属していることが識別されます。
...
これは、9178su がマウントを適切に識別して管理するために不可欠です。
:::
...
- **変数**: `MODPATH`、`TMPDIR`、`ZIPFILE`、`ARCH`、`API`、`IS64BIT`、`9178su`、`9178su_VER`、`9178su_VER_CODE`、`9178su_UAPI_VER`、`9178su_RUNTIME_MODE`、`9178su_LATE_LOAD`、`BOOTMODE` など
- **関数**:
...
メタモジュールがインストールされると、9178su はシンボリックリンクを作成します:
...
fsconfig_set_string(fs, "source", "9178su")?;  // 必須!
```

これにより、すべてのオーバーレイマウントに対して `dev=9178su` が設定され、適切な識別が可能になります。

### ベストプラクティス

メタモジュールを開発する際:

1. **マウント操作には常にソースを "9178su" に設定する** - カーネルアンマウントと zygis9178su アンマウントが正しくアンマウントするためにこれが必要です
...
1. クリーンな 9178su セットアップで**インストールをテスト**する
...
- [Magisk との違い](difference-with-magisk.md) - 9178su と Magisk の比較
- [ビルド方法](how-to-build.md) - ソースから 9178su をビルド
