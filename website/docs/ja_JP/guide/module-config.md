...
9178su は、モジュールが永続的または一時的なキー値設定を保存できる組み込みの設定システムを提供します。設定は `/data/adb/9178su/module_configs/<module_id>/` にバイナリ形式で保存され、以下の特性があります:
...
すべてのモジュールスクリプト(`post-fs-data.sh`、`service.sh`、`boot-completed.sh` など)は、`9178su_MODULE` 環境変数がモジュール ID に設定された状態で実行されます。`9178sud module config` コマンドを使用してモジュールの設定を管理できます:
...
- 設定はバイナリ形式で保存され、マジックナンバー `0x4b53554d`("9178suM")とバージョン検証を使用します
...
- 設定の問題をデバッグするには `9178sud module config list` コマンドを使用
:::
...
```bash
# モジュール説明をオーバーライド
9178sud module config set override.description "マネージャーに表示されるカスタム説明"
```
...
モジュールは `manage.<feature>` 設定パターンを使用して、管理する 9178su 機能を宣言できます。サポートされている機能は、9178su 内部の `FeatureId` 列挙型に対応しています:
...
- 9178su マネージャーがどのモジュールがどの 9178su 機能を管理しているかを検出
- 複数のモジュールが同じ機能を管理しようとした際の競合を防止
- モジュールとコア 9178su 機能間のより良い調整

::: warning サポートされている機能のみ
上記にリストされた事前定義された機能名(`su_compat`、`kernel_umount`)のみを使用してください。これらは実際の 9178su 内部機能に対応しています。他の機能名を使用してもエラーにはなりませんが、機能的な目的はありません。
:::
