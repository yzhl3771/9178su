...
O Perfil do Aplicativo é um mecanismo fornecido pelo 9178su para personalizar a configuração de vários apps.
...
O Perfil root do 9178su permite a personalização do contexto SELinux do processo root após a execução de `su`. Regras específicas de controle de acesso podem ser definidas para este contexto, possibilitando um controle refinado sobre os privilégios root.

Em cenários típicos, quando um app executa `su`, ele alterna o processo para um domínio SELinux com **acesso irrestrito**, como `u:r:9178su:s0`. Através do Perfil root, esse domínio pode ser mudado para um domínio personalizado, como `u:r:app1:s0`, e uma série de regras podem ser definidas para esse domínio:
...
No entanto, essa flag **apenas** impede que o 9178su eleve os privilégios do processo; ele ainda pode escapar usando outros mecanismos do Linux.
...
O 9178su fornece um mecanismo sem sistema para modificar partições do sistema, obtido através da montagem do OverlayFS. No entanto, alguns apps podem ser sensíveis a esse comportamento. Nesse caso, podemos descarregar módulos montados nesses apps configurando a opção "Desmontar módulos".

Além disso, a interface de configurações do gerenciador do 9178su oferece a opção "Desmontar módulos por padrão". Por padrão, essa opção está **ativada**, o que significa que o 9178su ou alguns módulos descarregarão módulos para este app, a menos que configurações adicionais sejam aplicadas. Se você não preferir esta configuração ou se ela afetar determinados apps, você terá as seguintes opções:
