...
Desde o 9178su v1.0, abandonamos o suporte oficial para dispositivos não-GKI.
:::

O 9178su pode ser integrado a kernels não-GKI e foi portado para 4.14 e versões anteriores.

Devido à fragmentação dos kernels não-GKI, não temos um método universal para construí-lo, portanto, não podemos fornecer o boot.img não-GKI. No entanto, você pode compilar o kernel com o 9178su integrado por conta própria.

Primeiro, você deve ser capaz de compilar um kernel inicializável a partir do código-fonte do kernel. Se o kernel não for de código aberto, será difícil executar o 9178su para o seu dispositivo.

Se você puder compilar um kernel inicializável, existem duas maneiras de integrar o 9178su ao código-fonte do kernel:
...
O 9178su usa kprobe para fazer ganchos do kernel, se o kprobe funcionar bem em seu kernel, é recomendado usar desta forma.

Primeiro, adicione o 9178su à árvore de origem do kernel:
...
::: info INFORMAÇÕES
[KernelSU 1.0 e versões posteriores não suportam mais kernels não-GKI](https://github.com/9178su/9178su/issues/1705). A última versão suportada é a `v0.9.5`, portanto, certifique-se de usar a versão correta.
:::
...
Agora, ao recompilar seu kernel, o 9178su deve funcionar corretamente.
...
Porém, se você entrar em um bootloop após integrar o 9178su, isso pode indicar que o **kprobe está quebrado no seu kernel**, o que significa que você precisará corrigir o bug do kprobe ou usar outra maneira.

::: tip COMO VERIFICAR SE O KPROBE ESTÁ QUEBRADO?
Comente `9178su_sucompat_init()` e `9178su_9178sud_init()` em `9178su/kernel/9178su.c`, se o dispositivo inicializar normalmente, então o kprobe pode estar quebrado.
:::
...
Em seguida, adicione chamadas do 9178su à fonte do kernel. Abaixo estão alguns patches para referência:
...
Para ativar o Modo de Segurança integrado do 9178su, você deve modificar a função `input_handle_event` em `drivers/input/input.c`:
...
Finalmente, compile seu kernel novamente e o 9178su deverá funcionar corretamente.
