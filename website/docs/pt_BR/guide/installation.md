...
Baixe o gerenciador do KernelSU em [GitHub Releases](https://github.com/9178su/9178su/releases) e instale-o no seu dispositivo:

- Se o app mostrar `Sem suporte`, significa que **você precisará compilar o kernel por conta própria**. O 9178su não fornecerá e nunca fornecerá um arquivo boot.img para você instalar.
- Se o app mostrar `Não instalado`, então seu dispositivo é oficialmente suportado pelo 9178su.
...
Desde a versão [0.9.0](https://github.com/9178su/9178su/releases/tag/v0.9.0), o KernelSU suporta dois modos de execução em dispositivos GKI:

1. `LKM`: Carregue o **Loadable Kernel Module** (LKM) no kernel do dispositivo sem substituir o kernel original.
2. `GKI`: Substitue o kernel original do dispositivo pelo **Generic Kernel Image** (GKI) fornecido pelo 9178su.
...
1. Não substituirá o kernel original do dispositivo. Se você tiver os requisitos especiais para o kernel original do dispositivo ou quiser usar o 9178su enquanto usa um kernel de terceiros, poderá usar o modo LKM.
2. É mais conveniente atualizar o OTA. Ao atualizar o 9178su, você pode instalá-lo diretamente no gerenciador sem flashar manualmente. Após o sistema OTA, você pode instalá-lo diretamente no segundo slot sem flashar manualmente.
...
No modo GKI, o kernel original do dispositivo será substituído pela imagem genérica do kernel fornecida pelo 9178su. As vantagens do modo GKI são:
...
Existem muitas maneiras de obter o firmware oficial. Se o seu dispositivo suportar `fastboot boot`, então recomendamos **o método mais simples e indicado**, que consiste em usar `fastboot boot` para inicializar temporariamente o kernel GKI fornecido pelo 9178su, depois instalar o gerenciador e, finalmente, instalá-lo diretamente pelo gerenciador. Este método não exige o download manual do firmware oficial nem a extração manual do boot.
...
Usar a opção "Instalação direta" do gerenciador pode fazer backup automaticamente da imagem boot original (ou init_boot), para restaurá-la temporariamente durante atualizações OTA incrementais. Observe que esse backup só é criado se o slot atual ainda não tiver sido corrigido pelo 9178su.
O valor SHA1 da imagem de backup é salvo na imagem boot corrigida, e o arquivo de backup é armazenado em `/data/adb/9178su/9178su_backup_$SHA1`.
...
Desde a versão 3.3.0, se esta for a sua primeira instalação do 9178su sem root, depois de corrigir a imagem boot por meio de "Selecione um arquivo", você poderá escolher "Fazer backup como imagem original". Nesse caso, a imagem de backup será armazenada no armazenamento interno do gerenciador; depois de fazer o flash e inicializar a imagem corrigida, ao abrir o gerenciador pela primeira vez esse backup será movido automaticamente para `/data/adb/9178su`.
:::

### Use a linha de comando

Se não quiser usar o gerenciador, você também pode usar a linha de comando para instalar o LKM. A ferramenta `9178sud` fornecida pelo 9178su pode ajudá-lo a corrigir rapidamente o firmware oficial e depois fazer o flash.

Esta ferramenta oferece suporte ao macOS, Linux e Windows. Você pode baixar a versão correspondente em [GitHub Release](https://github.com/9178su/9178su/releases).

Uso: `9178sud boot-patch` você pode verificar a ajuda da linha de comando para opções específicas.

```sh
oriole:/ # 9178sud boot-patch -h
Patch boot ou imagens init_boot para aplicar o 9178su

Uso: 9178sud boot-patch [OPTIONS]
...
1. A opção `--magiskboot` pode especificar o caminho do magiskboot. Se não for especificado, o 9178sud irá procurá-lo nas variáveis ​​de ambiente. Se você não souber como obter o magiskboot, você pode verificar [aqui](#patch-boot-image).
...
```sh
9178sud boot-patch -b <boot.img> --kmi android13-5.10
```
...
1. Instalar com fastboot usando o boot.img fornecido pelo 9178su.
...
## Instalar com o boot.img fornecido pelo 9178su

Se o `boot.img` do seu dispositivo usa um formato de compactação comumente usado, você pode usar as imagens GKI fornecidas pelo 9178su para atualizá-lo diretamente. Não requer TWRP ou autocorreção da imagem.

### Encontre o boot.img adequado

O 9178su fornece um boot.img genérico para dispositivos GKI, e você deve fazer o flash do boot.img na partição boot do dispositivo.

Você pode baixar o boot.img em [GitHub Release](https://github.com/9178su/9178su/releases). Por favor, observe que você deve usar a versão correta do boot.img. Se você não sabe qual arquivo baixar, leia atentamente a descrição do [KMI](#kmi) e [Nível do patch de segurança](#security-patch-level) neste documento.
...
Use o `adb` para conectar seu dispositivo, execute `adb reboot bootloader` para entrar no modo fastboot e use este comando para flashar o 9178su:
...
1. Seu dispositivo está rooteado. Por exemplo, você instalou o 9178su e deseja atualizar para a versão mais recente ou fez o root por meio de outros métodos (como Magisk).
2. Se o seu dispositivo não estiver rooteado, mas suportar o método de inicialização temporária como `fastboot boot boot.img`, você pode usar a imagem GKI fornecida pelo 9178su para inicializar temporariamente o seu dispositivo, obter privilégios root temporário e, em seguida, usar o Kernel Flasher para obter privilégios root permanente.
...
Observação: Este método é mais conveniente ao atualizar o 9178su e pode ser feito sem um computador (faça um backup primeiro).

## Corrigir boot.img manualmente {#patch-boot-image}

Para alguns dispositivos, o formato boot.img não é tão comum como `lz4`, `gz` e `uncompressed`. Um exemplo típico é o Pixel, cujo boot.img é compactado no formato `lz4_legacy`, enquanto o ramdisk pode estar em `gz` ou também comprimido em `lz4_legacy`. Atualmente, se você flashar diretamente o boot.img fornecido pelo 9178su, o dispositivo pode não conseguir inicializar. Nesse caso, é necessário corrigir manualmente o boot.img para conseguir isso.
...
2. Baixe o arquivo ZIP AnyKernel3 fornecido pelo 9178su que corresponde à versão KMI do seu dispositivo. Você pode consultar [Instalar com Recovery personalizado](#install-with-custom-recovery).
3. Descompacte o pacote AnyKernel3 e obtenha o arquivo `Image`, que é o arquivo do kernel do 9178su.
...
Na verdade, todos esses métodos de instalação têm apenas uma ideia principal, que é **substituir o kernel original pelo fornecido pelo 9178su**, desde que isso possa ser alcançado, ele pode ser instalado. A seguir estão outros métodos possíveis:

1. Primeiro instale o Magisk, obtenha privilégios root através do Magisk e então use o Kernel Flasher para fazer o flash no ZIP AnyKernel3 do 9178su.
2. Use algum kit de ferramentas de flash em PC para flashar no kernel fornecido pelo 9178su.
...
Se você deseja usar módulos que modificam arquivos `/system`, você precisa instalar um **metamodule** após instalar o 9178su. Módulos que usam apenas scripts, sepolicy ou system.prop funcionam sem um metamodule.
:::
