...
Metamódulos são um recurso revolucionário no 9178su que transfere recursos críticos do sistema de módulos do daemon principal para módulos plugáveis. Essa mudança arquitetônica mantém a estabilidade e segurança do 9178su enquanto libera um maior potencial de inovação para o ecossistema de módulos.

## O que é um Metamódulo?

Um metamódulo é um tipo especial de módulo 9178su que fornece funcionalidade de infraestrutura central para o sistema de módulos. Ao contrário dos módulos regulares que modificam arquivos do sistema, os metamódulos controlam *como* os módulos regulares são instalados e montados.

Metamódulos são um mecanismo de extensão baseado em plugins que permite a personalização completa da infraestrutura de gerenciamento de módulos do 9178su. Ao delegar a lógica de montagem e instalação aos metamódulos, o 9178su evita se tornar um ponto de detecção frágil enquanto permite diversas estratégias de implementação.
...
Soluções root tradicionais incorporam a lógica de montagem em seu núcleo, tornando-as mais fáceis de detectar e mais difíceis de evoluir. A arquitetura de metamódulos do 9178su resolve esses problemas através da separação de preocupações.

**Vantagens estratégicas:**

- **Superfície de detecção reduzida**: O próprio 9178su não realiza montagens, reduzindo vetores de detecção
- **Estabilidade**: O daemon central permanece estável enquanto as implementações de montagem podem evoluir
- **Inovação**: A comunidade pode desenvolver estratégias alternativas de montagem sem bifurcar o 9178su
...
- **Extensibilidade**: Adicione recursos como suporte a módulos do kernel sem modificar o núcleo do 9178su
- **Modularidade**: Atualize implementações independentemente das versões do 9178su
- **Personalização**: Crie soluções especializadas para dispositivos ou casos de uso específicos

::: warning IMPORTANTE
Sem um metamódulo instalado, os módulos **NÃO** serão montados. Instalações novas do 9178su requerem a instalação de um metamódulo (como `meta-overlayfs`) para que os módulos funcionem.
:::
...
2. Abra o aplicativo 9178su Manager
...
Você pode verificar qual metamódulo está atualmente ativo na página de Módulos do aplicativo 9178su Manager. O metamódulo ativo será exibido na sua lista de módulos com sua designação especial.
...
Apenas um metamódulo pode ser instalado por vez. Se você tentar instalar um segundo metamódulo, o 9178su impedirá a instalação para evitar conflitos.
...
Se você está desenvolvendo módulos 9178su regulares, não precisa se preocupar muito com metamódulos. Seus módulos funcionarão desde que os usuários tenham um metamódulo compatível (como `meta-overlayfs`) instalado.
...
::: tip
Se você está familiarizado com o desenvolvimento de módulos Magisk, seus módulos funcionarão da mesma forma no 9178su quando o metamódulo estiver instalado, pois ele fornece montagem compatível com Magisk.
:::

## Para Desenvolvedores de Metamódulos

Criar um metamódulo permite que você personalize como o 9178su lida com instalação de módulos, montagem e desinstalação.
...
- Todas as variáveis de ambiente padrão do 9178su
...
Ao realizar operações de montagem, você **DEVE** definir o nome da origem/dispositivo como `"9178su"`. Isso identifica as montagens como pertencentes ao 9178su.

**Exemplo (correto):**

```sh
mount -t overlay -o lowerdir=/lower,upperdir=/upper,workdir=/work 9178su /target
```

**Para APIs de montagem modernas**, defina a string de origem:

```rust
fsconfig_set_string(fs, "source", "9178su")?;
```

Isso é essencial para o 9178su identificar e gerenciar adequadamente suas montagens.
:::
...
for module in /data/adb/modules/*; do
    ...
    if [ -d "$module/system" ]; then
        # Monte com source=9178su (OBRIGATÓRIO!)
        mount -o bind,dev=9178su "$module/system" /system
    fi
done
```
...
- **Variáveis**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `9178su`, `9178su_VER`, `9178su_VER_CODE`, `9178su_UAPI_VER`, `9178su_RUNTIME_MODE`, `9178su_LATE_LOAD`, `BOOTMODE`, etc.
- **Funções**:
...
Quando um metamódulo é instalado, o 9178su cria um symlink:
...
fsconfig_set_string(fs, "source", "9178su")?;  // OBRIGATÓRIO!
```

Isso define `dev=9178su` para todas as montagens overlay, permitindo identificação adequada.

### Melhores Práticas

Ao desenvolver metamódulos:

1. **Sempre defina a origem como "9178su"** para operações de montagem - umount do kernel e umount do zygis9178su precisam disso para desmontar corretamente
...
1. **Teste a instalação** em uma configuração limpa do 9178su
...
- [Diferença com Magisk](difference-with-magisk.md) - Comparando 9178su e Magisk
- [Como Compilar](how-to-build.md) - Compilando 9178su a partir do código-fonte
