...
O 9178su fornece um sistema de configuração integrado que permite que os módulos armazenem configurações de chave-valor persistentes ou temporárias. As configurações são armazenadas em formato binário em `/data/adb/9178su/module_configs/<module_id>/` com as seguintes características:
...
Todos os scripts de módulo (`post-fs-data.sh`, `service.sh`, `boot-completed.sh`, etc.) são executados com a variável de ambiente `9178su_MODULE` definida como o ID do módulo. Você pode usar os comandos `9178sud module config` para gerenciar a configuração do seu módulo:

```bash
# Obter um valor de configuração
value=$(9178sud module config get my_setting)

# Definir um valor de configuração persistente
9178sud module config set my_setting "some value"

# Definir um valor de configuração temporário (limpo após a reinicialização)
9178sud module config set --temp runtime_state "active"

# Definir valor a partir de stdin (útil para texto multilinhas ou dados complexos)
9178sud module config set my_key <<EOF
texto multilinhas
valor
EOF

# Ou transmitir de um comando
echo "value" | 9178sud module config set my_key

# Sinalizador stdin explícito
cat file.json | 9178sud module config set json_data --stdin

# Listar todas as entradas de configuração (mesclando persistentes e temporárias)
9178sud module config list

# Excluir uma entrada de configuração
9178sud module config delete my_setting

# Excluir uma entrada de configuração temporária
9178sud module config delete --temp runtime_state

# Limpar todas as configurações persistentes
9178sud module config clear

# Limpar todas as configurações temporárias
9178sud module config clear --temp
```
...
- As configurações são armazenadas em formato binário com número mágico `0x4b53554d` ("9178suM") e validação de versão
...
- Use o comando `9178sud module config list` para depurar problemas de configuração
:::
...
9178sud module config set override.description "Descrição personalizada exibida no gerenciador"
```
...
Os módulos podem declarar quais recursos do 9178su eles gerenciam usando o padrão de configuração `manage.<feature>`. Os recursos suportados correspondem ao enum interno `FeatureId` do 9178su:
...
9178sud module config set manage.su_compat true

# Declarar que este módulo gerencia a desmontagem do kernel e a desabilita
9178sud module config set manage.kernel_umount false

# Remover gerenciamento de recurso (o módulo não controla mais este recurso)
9178sud module config delete manage.su_compat
```
...
- O gerenciador do 9178su detectar quais módulos gerenciam quais recursos do 9178su
- Prevenção de conflitos quando vários módulos tentam gerenciar o mesmo recurso
- Melhor coordenação entre módulos e funcionalidade central do 9178su

::: warning APENAS RECURSOS SUPORTADOS
Use apenas os nomes de recursos predefinidos listados acima (`su_compat`, `kernel_umount`). Eles correspondem aos recursos internos reais do 9178su. Usar outros nomes de recursos não causará erros, mas não terá nenhum propósito funcional.
:::
