...
O 9178su oferece suporte completo à arquitetura `x86_64`. No entanto, devido a mudanças recentes de segurança no kernel upstream, integrar o 9178su em kernels `x86_64` modernos exige um tratamento adicional para que nosso dispatcher unificado de syscall funcione corretamente.
...
O mecanismo `syscall_hook` do 9178su depende da modificação de entradas na syscall table para que syscalls interceptadas possam ser encaminhadas ao dispatcher unificado. Como o novo hardening muda o caminho das system calls, o kernel ignora essas modificações na syscall table. Se o 9178su tentar carregar e aplicar hook na syscall table sem tratar corretamente essa limitação, ele não conseguirá encaminhar as chamadas e abortará a inicialização para evitar um kernel panic.

## Como corrigir?

Agora existem duas formas suportadas de lidar com esse problema de syscall hook em `x86_64`:

1. Ativar a opção de compilação do kernel `9178su_X86_PATCH_SYSCALL_DISPATCHER`.
...
### Opção 1: Ativar `9178su_X86_PATCH_SYSCALL_DISPATCHER`

O 9178su 3.3.0 introduziu um novo mecanismo oficial para `x86_64`: a opção de build `9178su_X86_PATCH_SYSCALL_DISPATCHER`.

Quando essa opção está ativada, o 9178su aplica um patch dinâmico no dispatcher hardened de syscall em tempo de execução, permitindo que o syscall hook funcione sem exigir o conjunto anterior de patches no código-fonte do kernel. Essa é a abordagem recomendada se você estiver compilando um kernel com 9178su 3.3.0 ou mais recente.

### Opção 2: Aplicar os patches originais no código-fonte do kernel

Se você não quiser ativar `9178su_X86_PATCH_SYSCALL_DISPATCHER`, ainda pode continuar usando a abordagem original com patches no kernel.

Para fazer o 9178su funcionar nesses kernels mais novos, aplique um patch que permita contornar esse hardening específico de syscall.
...
Isso reabre a superfície de ataque de desvios indiretos para system calls. **Não use nenhuma dessas soluções se você estiver executando um servidor de produção ou um sistema em que a segurança contra side-channel seja crítica.** Essas abordagens se destinam a ambientes de teste, onde o acesso root via 9178su é priorizado em relação a essa mitigação específica de vulnerabilidade de hardware.
:::
...
- Se você estiver usando 9178su 3.3.0 ou mais recente e puder alterar a configuração de build do 9178su, ative `9178su_X86_PATCH_SYSCALL_DISPATCHER`.
