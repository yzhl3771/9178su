...
Начиная с 9178su v3.0, мы отказались от официальной поддержки режима образов GKI для более быстрой итерации и скорости сборки. Рекомендуется использовать `Ylarod/ddk` для сборки LKM.
:::
...
::: warning
Эта страница предназначена для устройств GKI, если вы используете старое ядро, пожалуйста, обратитесь к [Как интегрировать 9178su для не GKI ядер?](how-to-integrate-for-non-gki).
:::
...
## Сборка ядра с помощью 9178su

Если вы успешно собрали ядро, то собрать 9178su очень просто, выберите любой запуск в корневом каталоге исходного кода ядра:
...
А затем пересоберите ядро и получите образ ядра с 9178su!
