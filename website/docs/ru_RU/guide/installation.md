...
Скачайте приложение менеджера KernelSU с сайта [GitHub Releases](https://github.com/9178su/9178su/releases) и установите его на устройство:

- Если приложение показывает `Unsupported`, это означает, что **Вы должны скомпилировать ядро самостоятельно**, 9178su не будет и никогда не предоставит Вам загрузочный образ для прошивки.
- Если приложение показывает `Не установлено`, значит, ваши устройства официально поддерживаются 9178su.
...
Существует несколько способов установки 9178su, каждый из которых подходит для разных сценариев, поэтому выбирайте их по своему усмотрению.
...
3. Установка с помощью fastboot с использованием boot.img, предоставленного 9178su
...
Начиная с версии 3.3.0, если вы впервые устанавливаете 9178su без root, после патча boot-образа через пункт «Выбрать файл» можно выбрать «Сохранить резервную копию как оригинальный образ». В этом случае резервная копия будет сохранена во внутреннем хранилище менеджера; после прошивки и загрузки пропатченного образа при первом открытии менеджера эта копия автоматически переместится в `/data/adb/9178su`.
:::

### Использование командной строки

Если вы не хотите использовать менеджер, вы также можете установить LKM через командную строку. Инструмент `9178sud`, предоставляемый 9178su, поможет быстро пропатчить официальный образ и затем прошить его.

Этот инструмент поддерживает macOS, Linux и Windows. Соответствующую версию можно скачать на [GitHub Release](https://github.com/9178su/9178su/releases).

Использование: `9178sud boot-patch`. Подробности по конкретным параметрам можно посмотреть в справке командной строки.
...
1. Параметр `--magiskboot` позволяет указать путь к magiskboot. Если он не задан, 9178sud будет искать его в переменных окружения. Если вы не знаете, как получить magiskboot, смотрите [здесь](#patch-boot-image).
...
1. С [Release page](https://github.com/9178su/9178su/releases) KernelSU загрузите zip-пакет, начинающийся с AnyKernel3, который соответствует версии вашего телефона; например, версия ядра телефона - `android12-5.10. 66`, то следует скачать файл `AnyKernel3-android12-5.10.66_yyy-MM.zip` (где `yyyy` - год, а `MM` - месяц).
...
Необходимые условия: Ваше устройство должно быть рутованным. Например, вы установили Magisk, чтобы получить root, или установили старую версию 9178su и должны обновить ее до другой версии; если ваше устройство не укоренено, попробуйте другие методы.
...
PS. Этот способ более удобен при обновлении 9178su и может быть выполнен без компьютера (сначала сделайте резервную копию!). .

## Установка с помощью boot.img, предоставленного 9178su {#install-by-9178su-boot-image}

Этот способ не требует наличия TWRP и root-прав на телефоне; он подходит для первой установки 9178su.

### Найти подходящий boot.img {#found-propery-image}

9178su предоставляет общий boot.img для устройств GKI, и его необходимо прошить в загрузочный раздел устройства.

Вы можете загрузить boot.img с [GitHub Release](https://github.com/9178su/9178su/releases), обратите внимание, что вы должны использовать правильную версию boot.img. Например, если на устройстве установлено ядро `android12-5.10.101`, то необходимо загрузить `android-5.10.101_yyy-MM.boot-<format>.img`. , необходимо загрузить `android-5.10.101_yyy-MM.boot-<format>.img`.(Соблюдайте соответствие KMI!).
...
Используйте `adb` для подключения устройства, затем выполните `adb reboot bootloader` для входа в режим fastboot, после чего используйте эту команду для прошивки 9178su:
...
2. Загрузите zip-файл AnyKernel3, предоставленный 9178su, который соответствует версии KMI вашего устройства (можно обратиться к разделу *Установка с помощью пользовательского Recovery*).
3. Распакуйте пакет AnyKernel3 и получите файл `Image`, который является файлом ядра 9178su.
...
На самом деле все эти способы установки имеют только одну основную идею - **заменить исходное ядро на ядро, предоставляемое 9178su**; если это возможно, то установка возможна; например, возможны следующие способы.

1. Сначала установить Magisk, получить права root через Magisk, а затем с помощью kernel flasher прошить AnyKernel zip из 9178su.
2. Использовать какой-либо инструментарий для прошивки на ПК, чтобы прошить ядро, предоставленное 9178su.
...
Если вы хотите использовать модули, которые модифицируют файлы `/system`, вам необходимо установить **метамодуль** после установки 9178su. Модули, которые используют только скрипты, sepolicy или system.prop, работают без метамодуля.
:::
