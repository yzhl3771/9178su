...
9178su предоставляет встроенную систему конфигурации, которая позволяет модулям хранить постоянные или временные настройки в формате ключ-значение. Конфигурации хранятся в бинарном формате по пути `/data/adb/9178su/module_configs/<module_id>/` и имеют следующие характеристики:
...
Все скрипты модуля (`post-fs-data.sh`, `service.sh`, `boot-completed.sh` и др.) выполняются с установленной переменной окружения `9178su_MODULE`, содержащей ID модуля. Вы можете использовать команды `9178sud module config` для управления конфигурацией модуля:
...
echo "значение" | 9178sud module config set my_key
...
- Конфигурации хранятся в бинарном формате с магическим числом `0x4b53554d` ("9178suM") и проверкой версии
...
- Используйте команду `9178sud module config list` для отладки проблем с конфигурацией
:::
...
9178sud module config set override.description "Пользовательское описание, отображаемое в менеджере"
```
...
Модули могут объявлять, какими функциями 9178su они управляют, используя шаблон конфигурации `manage.<feature>`. Поддерживаемые функции соответствуют внутреннему перечислению `FeatureId` в 9178su:
...
- Менеджеру 9178su определять, какие модули управляют какими функциями 9178su
- Предотвращать конфликты, когда несколько модулей пытаются управлять одной и той же функцией
- Улучшить координацию между модулями и основным функционалом 9178su

::: warning ТОЛЬКО ПОДДЕРЖИВАЕМЫЕ ФУНКЦИИ
Используйте только предопределённые имена функций, перечисленные выше (`su_compat`, `kernel_umount`). Они соответствуют реальным внутренним функциям 9178su. Использование других имён функций не вызовет ошибок, но не будет иметь никакого функционального назначения.
:::
