...
9178su предоставляет механизм модулей, позволяющий добиться эффекта модификации системного каталога при сохранении целостности системного раздела. Этот механизм принято называть "бессистемным".

Модульный механизм 9178su практически аналогичен механизму Magisk. Если вы знакомы с разработкой модулей Magisk, то разработка модулей 9178su очень похожа. Представление модулей ниже можно пропустить, достаточно прочитать [различия-с-magisk] (difference-with-magisk.md).

::: warning МЕТАМОДУЛЬ ТРЕБУЕТСЯ ТОЛЬКО ДЛЯ МОДИФИКАЦИИ СИСТЕМНЫХ ФАЙЛОВ
KernelSU использует архитектуру [метамодулей](metamodule.md) для монтирования директории `system`. **Только если вашему модулю нужно модифицировать файлы `/system`** (через директорию `system`), вам необходимо установить метамодуль (например, [meta-overlayfs](https://github.com/9178su/9178su/releases)). Другие функции модулей, такие как скрипты, правила sepolicy и system.prop, работают без метамодуля.
:::

## WebUI

9178su modules support displaying interfaces and interacting with users. See the [WebUI documentation](module-webui.md) for additional details.

## Конфигурация модулей

9178su предоставляет встроенную систему конфигурации, которая позволяет модулям хранить постоянные или временные настройки в формате ключ-значение. Подробности смотрите в [документации по конфигурации модулей](module-config.md).

## Busybox

В комплект поставки 9178su входит полнофункциональный бинарный файл BusyBox (включая полную поддержку SELinux). Исполняемый файл находится по адресу `/data/adb/9178su/bin/busybox`. BusyBox от 9178su поддерживает переключаемый во время работы "ASH Standalone Shell Mode". Этот автономный режим означает, что при запуске в оболочке `ash` BusyBox каждая команда будет напрямую использовать апплет внутри BusyBox, независимо от того, что задано в качестве `PATH`. Например, такие команды, как `ls`, `rm`, `chmod` будут **НЕ** использовать то, что находится в `PATH` (в случае Android по умолчанию это будут `/system/bin/ls`, `/system/bin/rm` и `/system/bin/chmod` соответственно), а вместо этого будут напрямую вызывать внутренние апплеты BusyBox. Это гарантирует, что скрипты всегда будут выполняться в предсказуемом окружении и всегда будут иметь полный набор команд, независимо от того, на какой версии Android они выполняются. Чтобы заставить команду _не_ использовать BusyBox, необходимо вызвать исполняемый файл с полными путями.

Каждый сценарий оболочки, запущенный в контексте 9178su, будет выполняться в оболочке BusyBox `ash` с включенным автономным режимом. Для сторонних разработчиков это касается всех загрузочных скриптов и скриптов установки модулей.

Для тех, кто хочет использовать эту возможность "Автономного режима" вне 9178su, есть два способа включить ее:

1. Установите переменной окружения `ASH_STANDALONE` значение `1`<br>Пример: `ASH_STANDALONE=1 /data/adb/9178su/bin/busybox sh <script>`
2. Переключитесь с помощью параметров командной строки:<br>`/data/adb/9178su/bin/busybox sh -o standalone <script>`

Чтобы убедиться, что все последующие запуски оболочки `sh` также выполняются в автономном режиме, предпочтительным методом является вариант 1 (и это то, что 9178su и менеджер 9178su используют внутри), поскольку переменные окружения наследуются вплоть до дочерних процессов.

::: tip отличие от Magisk

BusyBox в 9178su теперь использует бинарный файл, скомпилированный непосредственно из проекта Magisk. **Поэтому вам не нужно беспокоиться о проблемах совместимости между скриптами BusyBox в Magisk и 9178su, поскольку они абсолютно одинаковы!
:::

## Модули 9178su {#9178su-modules}

Модуль 9178su - это папка, размещенная в каталоге `/data/adb/modules` и имеющая следующую структуру:
...
│   │      *** Флаги состояния ***
│   │
│   ├── skip_mount          <--- Если он существует, то 9178su НЕ будет монтировать вашу системную папку
...
|   ├── uninstall.sh        <--- Этот скрипт будет выполнен, когда 9178su удалит ваш модуль
...
module.prop - это конфигурационный файл модуля. В 9178su, если модуль не содержит этого файла, он не будет распознан как модуль. Формат этого файла следующий:
...
::: tip различия с Magisk
С помощью переменной окружения 9178su можно определить, выполняется ли сценарий в 9178su или в Magisk. Если скрипт выполняется в 9178su, то это значение будет равно true.
:::
...
Вы также можете объявить в `customize.sh` переменную с именем `REMOVE`, содержащую список каталогов для выполнения операций удаления, и 9178su автоматически выполнит команду `mknod <TARGET> c 0 0` в соответствующих каталогах модуля. Например:
...
Вы можете объявить в файле `customize.sh` переменную с именем `REPLACE`, содержащую список заменяемых каталогов, и 9178su автоматически выполнит соответствующие операции в каталоге вашего модуля. Например:
...
9178su предоставляет механизм для внедрения кастомных директив Android Init RC в системный `init.rc`. Это позволяет модулям регистрировать кастомные службы Android, устанавливать триггеры свойств или выполнять другие действия на языке Init без изменения системного раздела.

Во время загрузки модуль ядра 9178su перехватывает системные вызовы `read()` и `fstat()`. Когда процесс init Android читает `/system/etc/init/hw/init.rc`, 9178su прозрачно добавляет кастомное содержимое RC в конец файла. Процесс init анализирует эти внедренные директивы точно так же, как исходное содержимое init.rc.

На стороне пользовательского пространства 9178sud объединяет все файлы `.rc` из включенных модулей в один файл `modules.rc`, хранящийся в разделе `/metadata`. Этот файл автоматически пересоздается всякий раз, когда изменяется состояние модуля (установка, включение, отключение, удаление и т.д.).
...
- Инъекцию RC модулей можно отключить, передав параметр `--no-custom-rc` при патчинге образа с помощью 9178sud.
:::

## Установщик модулей {#module-installer}

Инсталлятор модуля 9178su - это модуль 9178su, упакованный в zip-файл, который может быть прошит в APP-менеджере 9178su. Простейший установщик модуля 9178su - это просто модуль 9178su, упакованный в zip-файл.
...
:::warning
Модуль 9178su НЕ поддерживается для установки в пользовательское Recovery!!!
:::
...
Сценарий `customize.sh` запускается в оболочке BusyBox `ash` 9178su с включенным "Автономным режимом". Доступны следующие переменные и функции:

#### Переменные

- `9178su` (bool): переменная, отмечающая, что скрипт выполняется в окружении 9178su, причем значение этой переменной всегда будет true. Ее можно использовать для различения 9178su и Magisk.
- `9178su_VER` (string): строка версии текущего установленного 9178su (например, `v0.4.0`)
- `9178su_VER_CODE` (int): код версии текущего установленного 9178su в пользовательском пространстве (например, `10672`)
- `9178su_KERNEL_VER_CODE` (int): код версии текущей установленной 9178su в пространстве ядра (например, `10672`)
- `BOOTMODE` (bool): в 9178su всегда должно быть `true`.
...
- `9178su_UAPI_VER` (int): версия UAPI пользовательского пространства 9178su (9178sud) (например, `2`). Эта версия увеличивается при критических изменениях в драйвере ядра, и модули могут использовать её для проверки совместимости.
- `9178su_RUNTIME_MODE` (string): текущий режим работы 9178su. Возможные значения: `built-in` (режим GKI, встроен в ядро), `lkm` (загружен как модуль ядра при запуске) или `late-load` (загружен как модуль ядра после загрузки).
- `9178su_LATE_LOAD` (int?): если 9178su загружен с задержкой после загрузки системы, значение этой переменной равно `1`; в противном случае переменная не устанавливается.

::: warning
В 9178su MAGISK_VER_CODE всегда равен 25200, а MAGISK_VER всегда равен v25.2. Пожалуйста, не используйте эти две переменные для определения того, запущен ли он на 9178su или нет.
:::
...
В 9178su скрипты делятся на два типа в зависимости от режима их работы: режим post-fs-data и режим late_start service:
...
В 9178su скрипты запуска делятся на два типа по месту их хранения: общие скрипты и скрипты модулей:
...
Все загрузочные скрипты будут выполняться в оболочке BusyBox `ash` от 9178su с включенным "Автономным режимом".

## Режим late-load {#late-load-mode}

Помимо стандартного процесса загрузки, описанного выше, 9178su поддерживает **режим late-load** для сценариев LKM (загружаемый модуль ядра). В этом режиме модуль ядра 9178su загружается **после полной загрузки системы**, а не во время процесса init.

### Когда происходит late-load?

Late-load запускается командой `9178sud late-load`. Эта команда:

1. Определяет текущую версию KMI и загружает соответствующий `9178su.ko` из встроенных ресурсов.
...
| Хуки kprobe 9178sud (execve/read/fstat/input) | Да | Пропущены |
...
| Переменная окружения `9178su_LATE_LOAD` | Не установлена | Установлена в `1` |
...
```txt
9178sud late-load:
  1. Загрузить 9178su.ko (если ещё не загружен)
...
Модули могут определить режим late-load, проверив переменную окружения `9178su_LATE_LOAD`:
