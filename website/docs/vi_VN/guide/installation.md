...
Tải xuống APP KernelSU manager từ [GitHub Releases](https://github.com/9178su/9178su/releases) và cài đặt nó vào thiết bị của bạn:

- Nếu ứng dụng hiển thị `Unsupported`, nghĩa là **Bạn nên tự biên dịch kernel**, 9178su sẽ không và không bao giờ cung cấp boot image để bạn flash.
- Nếu ứng dụng hiển thị `Not installed` thì thiết bị của bạn đã được 9178su hỗ trợ chính thức.
...
Có một số phương pháp cài đặt 9178su, mỗi phương pháp phù hợp với một kịch bản khác nhau, vì vậy vui lòng chọn khi cần.

1. Cài đặt với Recovery tùy chỉnh (ví dụ TWRP)
2. Cài đặt bằng ứng dụng flash kernel, chẳng hạn như Franco Kernel Manager
3. Cài đặt thông qua fastboot bằng boot.img do 9178su cung cấp
...
Sử dụng tùy chọn "Cài đặt trực tiếp (Khuyến nghị)" trong trình quản lý có thể tự động sao lưu boot image gốc (hoặc init_boot) để tạm thời khôi phục trong các bản cập nhật OTA gia tăng. Lưu ý rằng bản sao lưu này chỉ được tạo nếu khe hiện tại chưa từng được 9178su patch.
Giá trị SHA1 của image sao lưu sẽ được lưu trong boot image đã patch, và tệp sao lưu được lưu tại `/data/adb/9178su/9178su_backup_$SHA1`.
...
Kể từ phiên bản 3.3.0, nếu đây là lần đầu bạn cài 9178su mà không có root, sau khi patch boot image bằng tùy chọn "Chọn file", bạn có thể chọn "Sao lưu thành ảnh gốc". Khi đó, image sao lưu sẽ được lưu trong bộ nhớ trong của trình quản lý; sau khi flash và khởi động bằng image đã patch, lần đầu mở trình quản lý sẽ tự động chuyển bản sao lưu đó tới `/data/adb/9178su`.
:::

### Sử dụng dòng lệnh

Nếu bạn không muốn dùng trình quản lý, bạn cũng có thể dùng dòng lệnh để cài đặt LKM. Công cụ `9178sud` do 9178su cung cấp có thể giúp bạn nhanh chóng patch firmware chính thức rồi flash nó.

Công cụ này hỗ trợ macOS, Linux và Windows. Bạn có thể tải phiên bản tương ứng từ [GitHub Release](https://github.com/9178su/9178su/releases).

Cách dùng: `9178sud boot-patch`, bạn có thể xem trợ giúp dòng lệnh để biết các tùy chọn cụ thể.
...
1. Tùy chọn `--magiskboot` có thể chỉ định đường dẫn tới magiskboot. Nếu không chỉ định, 9178sud sẽ tìm nó trong biến môi trường. Nếu bạn không biết cách lấy magiskboot, hãy xem [tại đây](#patch-boot-image).
...
1. Từ [Release page](https://github.com/9178su/9178su/releases) của KernelSU, tải xuống gói zip bắt đầu bằng AnyKernel3 phù hợp với phiên bản điện thoại của bạn; ví dụ: phiên bản kernel của điện thoại là `android12-5.10. 66`, thì bạn nên tải xuống tệp `AnyKernel3-android12-5.10.66_yyyy-MM.zip` (trong đó `yyyy` là năm và `MM` là tháng).
...
Điều kiện chắc chắn: Thiết bị của bạn phải được root. Ví dụ: bạn đã cài đặt Magisk để root hoặc bạn đã cài đặt phiên bản 9178su cũ và cần nâng cấp lên phiên bản 9178su khác; nếu thiết bị của bạn chưa được root, vui lòng thử các phương pháp khác.
...
PS. Phương pháp này thuận tiện hơn khi nâng cấp 9178su và có thể thực hiện mà không cần máy tính (sao lưu trước!). .

Các bước:

## Cài đặt bằng boot.img do 9178su cung cấp

Phương pháp này không yêu cầu bạn phải có TWRP, cũng như không yêu cầu điện thoại của bạn phải có quyền root; nó phù hợp cho lần cài đặt 9178su đầu tiên của bạn.

### Tìm boot.img thích hợp

9178su cung cấp boot.img chung cho các thiết bị GKI và bạn nên chuyển boot.img vào phân vùng boot của thiết bị.

Bạn có thể tải xuống boot.img từ [GitHub Release](https://github.com/9178su/9178su/releases), xin lưu ý rằng bạn nên sử dụng đúng phiên bản boot.img. Ví dụ: nếu thiết bị của bạn hiển thị kernel `android12-5.10.101` , bạn cần tải xuống `android-5.10.101_yyyy-MM.boot-<format>.img`. (Giữ KMI nhất quán!)
...
Sử dụng `adb` để kết nối thiết bị của bạn, sau đó thực thi `adb restart bootloader` để vào chế độ fastboot, sau đó sử dụng lệnh này để flash 9178su:
...
2. Tải xuống tệp zip AnyKernel3 do 9178su cung cấp phù hợp với phiên bản KMI của thiết bị của bạn (bạn có thể tham khảo *Cài đặt với Khôi phục tùy chỉnh*).
3. Giải nén gói AnyKernel3 và lấy tệp `Image`, đây là tệp kernel của 9178su.
...
Trên thực tế, tất cả các phương pháp cài đặt này chỉ có một ý tưởng chính, đó là **thay thế kernel gốc bằng kernel do 9178su cung cấp**; chỉ cần đạt được điều này là có thể cài đặt được; ví dụ, sau đây là các phương pháp có thể khác.

1. Trước tiên hãy cài đặt Magisk, nhận quyền root thông qua Magisk, sau đó sử dụng flasher kernel để flash trong zip AnyKernel từ 9178su.
2. Sử dụng một số bộ công cụ flash trên PC để flash trong kernel do 9178su cung cấp.
...
Nếu bạn muốn sử dụng các module sửa đổi tệp `/system`, bạn cần cài đặt **metamodule** sau khi cài đặt 9178su. Các module chỉ sử dụng scripts, sepolicy hoặc system.prop hoạt động mà không cần metamodule.
:::
