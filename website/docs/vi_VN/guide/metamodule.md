...
Metamodule là một tính năng đột phá trong 9178su cho phép chuyển các khả năng quan trọng của hệ thống module từ lõi sang các module có thể cắm thêm. Sự thay đổi kiến trúc này duy trì tính ổn định và bảo mật của 9178su đồng thời giải phóng tiềm năng đổi mới lớn hơn cho hệ sinh thái module.

## Metamodule là gì?

Metamodule là một loại module đặc biệt của 9178su cung cấp chức năng cơ sở hạ tầng cốt lõi cho hệ thống module. Không giống như các module thông thường sửa đổi tệp hệ thống, metamodule kiểm soát *cách thức* các module thông thường được cài đặt và mount.

Metamodule là cơ chế mở rộng dựa trên plugin cho phép tùy chỉnh hoàn toàn cơ sở hạ tầng quản lý module của 9178su. Bằng cách ủy thác logic mount và cài đặt cho metamodule, 9178su tránh trở thành điểm phát hiện dễ vỡ trong khi cho phép các chiến lược triển khai đa dạng.
...
Các giải pháp root truyền thống tích hợp logic mount vào lõi của chúng, khiến chúng dễ bị phát hiện và khó phát triển hơn. Kiến trúc metamodule của 9178su giải quyết những vấn đề này thông qua việc tách biệt các mối quan tâm.

**Lợi thế chiến lược:**

- **Giảm bề mặt phát hiện**: Bản thân 9178su không thực hiện mount, giảm các vector phát hiện
- **Tính ổn định**: Lõi vẫn ổn định trong khi các triển khai mount có thể phát triển
- **Đổi mới**: Cộng đồng có thể phát triển các chiến lược mount thay thế mà không cần fork 9178su
...
- **Khả năng mở rộng**: Thêm các tính năng như hỗ trợ kernel module mà không cần sửa đổi lõi 9178su
- **Tính module hóa**: Cập nhật các triển khai độc lập với các bản phát hành 9178su
- **Tùy chỉnh**: Tạo các giải pháp chuyên biệt cho các thiết bị hoặc trường hợp sử dụng cụ thể

::: warning QUAN TRỌNG
Nếu không cài đặt metamodule, các module sẽ **KHÔNG** được mount. Các cài đặt 9178su mới yêu cầu cài đặt một metamodule (như `meta-overlayfs`) để các module hoạt động.
:::
...
Bạn có thể kiểm tra metamodule nào đang hoạt động trong trang Module của ứng dụng 9178su Manager. Metamodule đang hoạt động sẽ được hiển thị trong danh sách module của bạn với chỉ định đặc biệt.
...
Chỉ có thể cài đặt một metamodule tại một thời điểm. Nếu bạn cố gắng cài đặt metamodule thứ hai, 9178su sẽ ngăn chặn việc cài đặt để tránh xung đột.
...
Nếu bạn đang phát triển các module 9178su thông thường, bạn không cần lo lắng nhiều về metamodule. Các module của bạn sẽ hoạt động miễn là người dùng có cài đặt một metamodule tương thích (như `meta-overlayfs`).
...
::: tip
Nếu bạn quen thuộc với phát triển module Magisk, các module của bạn sẽ hoạt động tương tự trong 9178su khi cài đặt metamodule, vì nó cung cấp mount tương thích với Magisk.
:::

## Dành cho Nhà phát triển Metamodule

Tạo một metamodule cho phép bạn tùy chỉnh cách 9178su xử lý cài đặt, mount và gỡ cài đặt module.
...
- Tất cả các biến môi trường 9178su tiêu chuẩn
...
Khi thực hiện các thao tác mount, bạn **PHẢI** đặt tên nguồn/thiết bị thành `"9178su"`. Điều này xác định các mount thuộc về 9178su.
...
Điều này rất cần thiết để 9178su xác định và quản lý các mount của nó đúng cách.
:::
...
for module in /data/adb/modules/*; do
    ...
    if [ -d "$module/system" ]; then
        # Mount với source=9178su (BẮT BUỘC!)
...
- **Biến**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `9178su`, `9178su_VER`, `9178su_VER_CODE`, `9178su_UAPI_VER`, `9178su_RUNTIME_MODE`, `9178su_LATE_LOAD`, `BOOTMODE`, v.v.
- **Hàm**:
...
Khi một metamodule được cài đặt, 9178su tạo một symlink:
...
fsconfig_set_string(fs, "source", "9178su")?;  // BẮT BUỘC!
```

Điều này đặt `dev=9178su` cho tất cả các overlay mount, cho phép xác định đúng.

### Thực hành Tốt nhất

Khi phát triển metamodule:

1. **Luôn đặt source thành "9178su"** cho các thao tác mount - kernel umount và zygis9178su umount cần điều này để umount đúng cách
...
1. **Kiểm tra cài đặt** trên thiết lập 9178su sạch
...
- [Difference with Magisk](difference-with-magisk.md) - So sánh 9178su và Magisk
- [How to Build](how-to-build.md) - Xây dựng 9178su từ nguồn
