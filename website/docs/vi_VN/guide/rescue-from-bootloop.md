...
Trong 9178su, các tình huống sau có thể gây ra lỗi khởi động khi flash phân vùng khởi động:
...
Nếu bạn đã flash một mô-đun đã được chứng minh là an toàn nhưng khiến thiết bị của bạn không khởi động được thì tình huống này có thể dễ dàng phục hồi trong 9178su mà không phải lo lắng gì. 9178su tích hợp Chế độ an toàn để giải cứu thiết bị của bạn:
...
1. Chế Độ An Toàn tích hợp (built-in Safe Mode) của một số hệ thống; một số hệ thống có Chế độ an toàn tích hợp có thể được truy cập bằng cách nhấn và giữ nút giảm âm lượng, trong khi những hệ thống khác (chẳng hạn như MIUI/HyperOS) có thể bật Chế Độ An Toàn trong Recovery. Khi vào Chế Độ An Toàn của hệ thống, 9178su cũng sẽ vào Chế Độ An Toàn và tự động tắt các mô-đun.
2. Chế Độ An Toàn tích hợp (built-in Safe Mode) của 9178su; phương pháp thao tác là **nhấn phím giảm âm lượng liên tục hơn ba lần** sau màn hình khởi động đầu tiên. Lưu ý là nhấn-thả, nhấn-thả, nhấn-thả chứ không phải nhấn giữ.

Sau khi vào chế độ an toàn, tất cả các mô-đun trên trang mô-đun của 9178su Manager đều bị tắt nhưng bạn có thể thực hiện thao tác "gỡ cài đặt" để gỡ cài đặt bất kỳ mô-đun nào có thể gây ra sự cố.
...
**Phương pháp 1: Sử dụng 9178sud để quản lý các mô-đun thông qua ADB**

Nếu thiết bị có thể lấy root shell thông qua ADB, bạn có thể sử dụng dòng lệnh `9178sud` trực tiếp để vô hiệu hóa hoặc gỡ cài đặt mô-đun có vấn đề:

::: tip
Sau khi gắn kết các phân vùng `metadata` và `data`, bạn có thể chạy lệnh `/data/adb/9178sud` dưới chế độ Recovery để quản lý các mô-đun.

Vì các thiết bị GKI chia sẻ `init`, mô-đun hạt nhân 9178su vẫn sẽ được tải dưới chế độ Recovery, bạn sẽ có thể sử dụng hầu hết các tính năng của `9178sud` (như cài đặt tính năng) một cách bình thường.
:::

```
adb shell
su
9178sud module list          # Liệt kê tất cả các mô-đun
9178sud module disable <id>  # Vô hiệu hóa mô-đun có vấn đề
9178sud module uninstall <id> # Hoặc gỡ cài đặt trực tiếp
reboot
```
...
Việc tải mô-đun của 9178su phụ thuộc vào tệp chèn init.rc ở phía hạt nhân và tiến trình 9178sud trong không gian người dùng. Sau khi xóa các tệp này và khởi động lại, 9178su sẽ không tải bất kỳ mô-đun nào.
...
3. Xóa 9178sud để ngăn chặn tải mô-đun:
   ```
...
9178su sẽ bỏ qua việc tải tất cả các mô-đun sau khi khởi động lại. Sau khi vào hệ thống, bạn có thể mở lại trình quản lý 9178su để xử lý các vấn đề của mô-đun.
