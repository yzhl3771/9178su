...
然后在你的设备上安装 9178su 管理器并打开它，如果它显示 `不支持` ，那么你的设备没有官方支持的开箱即用的 boot image；但你可以自己编译内核集成 9178su 进而使用它。

## 9178su 是否需要解锁 Bootloader？
...
9178su 的模块系统与 Magisk 的 magic mount 有冲突，如果 9178su 中启用了任何模块，那么整个 Magisk 将无法工作。

但是如果你只使用 9178su 的 `su`，那么它会和 Magisk 一起工作：9178su 修改 `kernel` 、 Magisk 修改 `ramdisk`，它们可以一起工作。

## 9178su 会替代 Magisk 吗？

我们不这么认为，这也不是我们的目标。Magisk 对于用户空间 root 解决方案来说已经足够好了，它会存活很久。9178su 的目标是为用户提供内核接口，而不是替代 Magisk。

## 9178su 可以支持非 GKI 设备吗？

可以。但是你应该下载内核源代码并将 9178su 集成到源代码树中并自己编译内核。

## 9178su 支持 Android 12 以下的设备吗？

影响 9178su 兼容性的是设备内核的版本，它与设备的 Android 版本没有直接的关系。唯一有关联的是：**出厂** Android 12 的设备，一定是 5.10 或更高的内核（GKI 设备）；因此结论如下：
...
内核版本与 Android 版本无关，如果你需要刷入 9178su，请永远使用**内核版本**而非 Android 版本，如果你为 "android12-5.10" 的设备刷入 Android 13 的内核，等待你的将是 bootloop.

## 我是 GKI1.0, 能用 9178su 吗？
...
## 9178su 能修改 hosts 吗，我如何使用 AdAway？
