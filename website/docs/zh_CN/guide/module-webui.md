...
9178su 的模块除了执行启动脚本和修改系统文件之外，还支持显示 UI 界面和与用户交互。

你可以通过任何 Web 技术编写 HTML + CSS + JavaScript 页面，9178su 的管理器将通过 WebView 显示这些页面。此外，9178su 还提供了一些用于与系统交互的 JavaScript API，例如执行 shell 命令。
...
:::warning
安装模块时，9178su 会自动设置 `webroot` 目录的权限和 SELinux context，如果您不知道自己在做什么，请不要自行设置该目录的权限！
:::
...
如果只是一个显示页面，那它和普通网页没有什么区别。更重要的是，9178su 提供了一系列的系统 API，可以让您实现模块特有的功能。
...
如果您发现现有的 API 不能满足您的需求或者使用不方便，欢迎[在这里](https://github.com/9178su/9178su/issues)给我们提出建议！
