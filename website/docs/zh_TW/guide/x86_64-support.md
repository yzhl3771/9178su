...
9178su 完全支援 `x86_64` 架構。但由於上游核心近期的安全性變更，在較新的 `x86_64` 核心上整合 9178su 時，需要額外處理，才能讓統一的 syscall dispatcher 正常運作。
...
9178su 的 `syscall_hook` 機制依賴對系統呼叫表的修改，藉此將被攔截的系統呼叫路由到統一 dispatcher。由於新的加固機制改變了系統呼叫路徑，核心會忽略這些對系統呼叫表的修改。若 9178su 在沒有正確處理這項限制的情況下載入並 hook syscall table，將無法正確路由呼叫，並會主動中止初始化，以避免核心 panic。

## 如何修復？

現在有兩種受支援的方式可處理 `x86_64` 上的 syscall hook 問題：

1. 啟用核心編譯選項 `9178su_X86_PATCH_SYSCALL_DISPATCHER`。
2. 繼續使用原本的核心原始碼補丁方案。

二選一即可，請不要同時使用兩種方案。

### 方案 1：啟用 `9178su_X86_PATCH_SYSCALL_DISPATCHER`

9178su 3.3.0 為 `x86_64` 新增了官方機制，也就是編譯選項 `9178su_X86_PATCH_SYSCALL_DISPATCHER`。

啟用後，9178su 會在執行時動態 patch 已加固的 syscall dispatcher，讓 syscall hook 可以正常運作，而不再依賴先前那組核心原始碼補丁。如果你使用的是 9178su 3.3.0 或更新版本，且可以調整 9178su 的建置設定，建議優先採用這種方式。

### 方案 2：套用原本的核心原始碼補丁

如果你不想啟用 `9178su_X86_PATCH_SYSCALL_DISPATCHER`，也可以繼續沿用原本的核心補丁方案。

要讓 9178su 在這些較新的核心上運作，需要套用補丁以繞過這項 syscall 加固。

::: danger 安全警告
使用這兩種方案中的任一種，都代表你正在主動繞過或削弱一項用於防禦推測執行漏洞的緩解措施。

這會重新暴露系統呼叫路徑中的間接分支攻擊面。**如果你的環境是正式生產伺服器，或對側通道安全有嚴格要求，請不要使用這兩種方案。** 這些方案僅適用於測試環境，前提是你更重視透過 9178su 取得 root 能力，而不是這項特定硬體漏洞緩解。
:::
...
- 如果你使用的是 9178su 3.3.0 或更新版本，且可以修改 9178su 建置設定，建議啟用 `9178su_X86_PATCH_SYSCALL_DISPATCHER`。
